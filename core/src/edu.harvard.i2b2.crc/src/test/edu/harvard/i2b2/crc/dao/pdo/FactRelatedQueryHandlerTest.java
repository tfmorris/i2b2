	...
	private static String testFileDir = null;
	 ...
	 @BeforeClass
		...
		public  void testPdoBuildQuery() throws Exception { 
			String filename = testFileDir + "/edu.harvard.i2b2.crc.dao.pdo/pdo_query.xml";
			...
			GetPDOFromInputListRequestType pdoRequestType = (GetPDOFromInputListRequestType)unWrapHelper.getObjectByClass(reqMsgType.getMessageBody().getAny(),GetPDOFromInputListRequestType.class);
			...
			if (dataSourceLookup.getServerType().equalsIgnoreCase(DAOFactoryHelper.SQLSERVER)) { 
				pdoQueryHandler = new SQLServerFactRelatedQueryHandler(dataSourceLookup,pdoRequestType.getInputList(),
						pdoRequestType.getFilterList(),pdoRequestType.getOutputOption());
			} else if (dataSourceLookup.getServerType().equalsIgnoreCase(DAOFactoryHelper.ORACLE)) { 
				pdoQueryHandler = new FactRelatedQueryHandler(dataSourceLookup,pdoRequestType.getInputList(),
						pdoRequestType.getFilterList(),pdoRequestType.getOutputOption());
			}
			
			FilterListType filterListType = pdoRequestType.getFilterList();
			PanelType panel = filterListType.getPanel().get(0);
			String pdoSql = pdoQueryHandler.buildQuery(panel, PdoQueryHandler.PLAIN_PDO_TYPE);
