...
public class QueryInfoBean implements SessionBean {
	 ...
	 * 
	 * @param userRequestType user_id 
	 * 
	 * @return String publish response XML
	 */
	public MasterResponseType getQueryMasterListFromUserId(UserRequestType userRequestType) {
			...
			List<QtQueryMaster> masterList  = queryMasterDao.getQueryMasterByUserId(userId,fetchSize);
	 ...
	 * @ejb.transaction type="Required"
	 * 
	 * @param userRequestType group_id
	 * 
	 * @return String publish response XML
	 */
	public MasterResponseType getQueryMasterListFromGroupId(UserRequestType userRequestType) {
		String groupId = userRequestType.getGroupId();
		int fetchSize = userRequestType.getFetchSize();
		QueryMasterDao queryMasterDao = new QueryMasterDao();
		List<QtQueryMaster> masterList  = queryMasterDao.getQueryMasterByGroupId(groupId,fetchSize);
		MasterResponseType masterResponseType = buildMasterResponseType(masterList);  
		return masterResponseType;
	}
	 ...
	 * @return String publish response XML
	 */
		public MasterResponseType getRequestXmlFromMasterId(String userId, MasterRequestType masterRequestType) throws I2B2Exception {
			String queryMasterId =  masterRequestType.getQueryMasterId();
			
			QueryMasterDao queryMasterDao = new QueryMasterDao();
			QtQueryMaster qtQueryMaster = queryMasterDao.getQueryDefinition(queryMasterId);
			...
			if (qtQueryMaster != null) { 
				...
				if (requestXml != null) {
					...
					try {
						...
						JAXBElement jaxbElement  = (JAXBElement)unMarshaller.unmarshal(new StringReader(requestXml));
						QueryDefinitionType queryDefinition = (QueryDefinitionType)jaxbElement.getValue();
						
						
						//marshall to dom
						JAXBContext jc = JAXBContext.newInstance(edu.harvard.i2b2.crc.datavo.setfinder.query.QueryDefinitionType.class);
						Marshaller m = jc.createMarshaller();
						DocumentBuilderFactory f = DocumentBuilderFactory.newInstance();
						DocumentBuilder builder = f.newDocumentBuilder();
						doc = builder.newDocument();
						m.marshal((new edu.harvard.i2b2.crc.datavo.setfinder.query.ObjectFactory()).createQueryDefinition(queryDefinition), doc);
						*/
						
						 doc = edu.harvard.i2b2.common.util.xml.XMLUtil.convertStringToDOM(requestXml);
						log.debug("query definition xml prefix " + doc.getDocumentElement().getPrefix());
						requestXmlType.getContent().add(doc.getDocumentElement());
					} catch (Exception i2b2) {
						i2b2.printStackTrace();
						throw new I2B2Exception("Error converting request xml to dom " + i2b2.getMessage(),i2b2);
					}
		 ...
		 */
		public MasterResponseType deleteQueryMaster(String userId, String masterId) throws I2B2Exception {
		 ...
		 */
		public MasterResponseType renameQueryMaster(String userId, String masterId,String queryNewName) throws I2B2Exception {
	...
	//private functions
	//-------------------------------------------------
	private MasterResponseType buildMasterResponseType(List<QtQueryMaster> masterList) { 
		MasterResponseType masterResponseType = new MasterResponseType();
		//masterResponseType
		...
		for (QtQueryMaster queryMaster : masterList) {
			...
			queryMasterType.setCreateDate(dtoFactory.getXMLGregorianCalendar(createDate.getTime()));
			java.util.Date deleteDate = queryMaster.getDeleteDate();
			if (deleteDate != null) { 
				queryMasterType.setDeleteDate(dtoFactory.getXMLGregorianCalendar(deleteDate.getTime()));
			} 
