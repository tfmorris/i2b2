...
import edu.harvard.i2b2.analysis.ui.QueryClient;


/**
 * The SelectServiceDriver class controls calls to/from 
...
public class SelectServiceDriver {
	...
	public static final String THIS_CLASS_NAME = SelectServiceDriver.class.getName();
	private static final Log log = LogFactory
    .getLog(SelectServiceDriver.class);
	 ...
	 *
	 * @param selectParameters   a document describing the input parameter to the Select Web service
	 * @return a document containing the result of the Select web service transaction
	 */
	 ...
	 *
	 * @param code   the concept code we are inquiring about
	...
	/**
	 * Test code to print the result document from the Select web service
	 *   in XML format
	 *
	 * @param resultDoc   a document describing the results returned by the Select Web service
	 * 
	 */
    private void printResultsAsXML(org.w3c.dom.Document resultDoc){
    	try{
    	    ...
    	    log.debug(new XMLOutputter().outputString(jresultDoc));
    	}catch(Exception e) {
			Logger.getLogger(THIS_CLASS_NAME).severe(e.getMessage());
			e.printStackTrace();				
    	}
    ...
    public String getNameFromDocument (org.w3c.dom.Document resultDoc){	
    	...
    	try {
    	    ...
    	    Iterator iterator = doc.getRootElement().getChildren("patientData", ns).iterator();
	...
	public static void main(String args[]){
		System.setProperty("ontologywebservice", "http://localhost:8080/i2b2/services/Select");
