...
import edu.harvard.i2b2.analysis.ui.DateConstraintEditorText;
import edu.harvard.i2b2.analysis.ui.NumericValueEditorText;
...
public class ConceptTableModel implements KTableModel {
	...
	try {
	    ...
	    data.dimcode(c_dimcode);
	    data.hasValue(hasValue);
	    data.xmlContent(conceptText);
	    data.columnName(c_columnname);
	    data.tableName(c_table);
	    data.columnDataType(c_columndatatype);
	    data.operator(c_operator);
