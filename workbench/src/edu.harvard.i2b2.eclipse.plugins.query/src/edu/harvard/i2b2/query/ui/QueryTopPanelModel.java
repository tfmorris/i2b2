...
public class QueryTopPanelModel {
	...
	public void setTopPanel(QueryTopPanel p) {
		topPanel = p;
	}
	...
	public String getTmpQueryName() {
		...
		Calendar cldr = Calendar.getInstance(TimeZone
				.getTimeZone("America/New_York"));
		name += "@"/*
		 * +addZero(cldr.get(Calendar.MONTH)+1)+"-"+addZero(cldr.get(Calendar
		 * .DAY_OF_MONTH))+"-" +addZero(cldr.get(Calendar.YEAR))+" "
		 */
	...
	private void redrawPanels(String xmlstr) {
		...
		try {
			...
			if (xmlTest != null && xmlTest.getClass().getSimpleName()
					.equalsIgnoreCase("QueryMasterType")) {

				QueryMasterType queryMasterType = (QueryMasterType) new JAXBUnWrapHelper()
				.getObjectByClass(dndType.getAny(), QueryMasterType.class);
				...
				.getObjectByClass(dndType.getAny(), QueryDefinitionType.class);
			}
			...
			for (int i = 0; i < numOfPanels; i++) {
				...
				// on the items
				// TODO recusive into all constrainbydate, not the first
				...
				// java.awt.EventQueue.invokeLater(new Runnable() {
				// public void run() {
				topPanel.setQueryName(" Query Name: " + name);
