...
public class NumericValueConstrainFrame extends javax.swing.JFrame {
	private QueryConceptTreePanel parent_;

	/** Creates new form ValueConstrainFrame */
	public NumericValueConstrainFrame(QueryConceptTreePanel parent) {
		parent_ = parent;
		...
		if (!parent.currentData().valuePropertyData().okToUseValueFlag()) {
			jFlagRadioButton.setEnabled(false);
		}

		if (!parent.currentData().valuePropertyData().okToUseValue()) {
			jNumericRadioButton.setEnabled(false);
		}

		// jUnitsComboBox.removeAllItems();
		for (int i = 0; i < parent.currentData().valuePropertyData().units
				.size(); i++) {
			jUnitsComboBox
					.addItem(parent.currentData().valuePropertyData().units
							.get(i));
		}
		setPreviousValues();
	...
	private void setPreviousValues()
	{
		...
		{
			jOperatorComboBox.setSelectedItem(parent_.currentData().valuePropertyData().operator());

			jNumericRadioButton.setSelected(true);
			jNumericRadioButtonActionPerformed(null);
			jOperatorComboBoxActionPerformed(null);
			if ( parent_.currentData().valuePropertyData().operator().equals("BETWEEN"))
			{
				jLowValueTextField.setText(parent_.currentData().valuePropertyData().lowValue());
				jHighTextField.setText(parent_.currentData().valuePropertyData().highValue());
			} else //if ( parent_.currentData().valuePropertyData().value().equals("L"))
			{
				jValueTextField.setText(parent_.currentData().valuePropertyData().value());
			}
		...
		if (parent_.currentData().hasValue() && parent_.currentData().valuePropertyData().unit() != null)
		{
			jUnitsComboBox.setSelectedItem(parent_.currentData().valuePropertyData().unit());
		}
	...
	private void initComponents() {
		...
		jNoValueButton.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
	...
	private void jOKButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (jNoValueRadioButton.isSelected()) {
			...
			if (jFlagComboBox.getSelectedIndex() == 0) {
				data.valueName(" = HIGH");
				data.valuePropertyData().value("H");
			} else {
				data.valueName(" = LOW");
				data.valuePropertyData().value("L");
			}
		} else if (jNumericRadioButton.isSelected()) {
			data.valuePropertyData().noValue(false);
			data.valuePropertyData().useValueFlag(false);
			data.valuePropertyData().useNumericValue(true);
			data.valuePropertyData().value(jValueTextField.getText());

			String operator = (String) jOperatorComboBox.getSelectedItem();
			data.valuePropertyData().operator(operator);

			// deal with between...
			if (operator.equalsIgnoreCase("between")) {
				data.valuePropertyData().lowValue(jLowValueTextField.getText());
				data.valuePropertyData().highValue(jHighTextField.getText());
				data.valueName("(" + data.valuePropertyData().lowValue()
						+ " - " + data.valuePropertyData().highValue() + ")");
			} else {
				data.valueName(getOperator(data.valuePropertyData().operator())
						+ data.valuePropertyData().value());
			}
		}

		if (jUnitsComboBox.getSelectedItem() != null) {
			data.valuePropertyData().unit(
					(String) jUnitsComboBox.getSelectedItem());
		}

		data.name(parent_.currentData().titleName() + parent_.currentData().valueName());
