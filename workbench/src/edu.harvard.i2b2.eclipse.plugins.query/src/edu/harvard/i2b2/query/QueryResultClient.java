...
public class QueryResultClient {
	...
	public static String getCount(String sessionId) {
		try {
			...
			basicAuthentication.setUsername(System.getProperty("user"));
			basicAuthentication.setPassword(System.getProperty("pass"));
			...
			OMElement attachmentElement = (OMElement)result.getFirstChildWithName(new QName("attachment"));
            if (attachmentElement != null) { 
            	OMText dataElement = (OMText)attachmentElement.getFirstOMChild();
            	dataElement.setOptimize(true);
            	
            	//System.out.println("Dataelement " + dataElement.getText());
                DataHandler attachDataHandler = (DataHandler)dataElement.getDataHandler();
                BufferedReader reader = new BufferedReader(new InputStreamReader(attachDataHandler.getInputStream()));
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			OMElement payload = getQueryPayLoad("1158085997546");
			Options options = new Options();
            ...
            if (attachmentElement != null) { 
                ...
                DataHandler attachDataHandler = (DataHandler)dataElement.getDataHandler();
                BufferedReader reader = new BufferedReader(new InputStreamReader(attachDataHandler.getInputStream()));
                //System.out.println("Data read " + reader.readLine());
            }
		} catch (AxisFault axisFault) {
			axisFault.printStackTrace();
		}
