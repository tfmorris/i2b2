...
public class TimeConstrainFrame extends javax.swing.JFrame {
	...
	private ConceptTreePanel parentPanel;

	/** Creates new form QueryConstrainFrame */
	public TimeConstrainFrame(ConceptTreePanel parent) {
		initComponents();
		// mode = Mode;
		parentPanel = parent;
		...
		if (parent.getClass().getName()
				.equals(ConceptTreePanel.class.getName())) {
	...
	private void jOKButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (this.jFromDateCheckBox.isSelected()) {
			...
			parentPanel.data()
					.startDay(fromCalendar.get(Calendar.DAY_OF_MONTH));
