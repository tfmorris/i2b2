...
import edu.harvard.i2b2.query.data.QueryResultData;
import edu.harvard.i2b2.query.datavo.QueryJAXBUtil;
import edu.harvard.i2b2.query.ui.QueryConceptTreePanel.QueryDataTransferable;
...
public class QueryTopPanel extends javax.swing.JPanel {
	...
	if (jGetPatientSetCheckBox.isSelected()
		|| jShowTimelineCheckBox.isSelected()) {
		...
		if (response != null) {
			...
			if (status.equalsIgnoreCase("DONE")) {
				...
				for (int i = 0; i < masterInstanceResultResponseType
					...
					System.out
						.println("Generated XML document request: "
							+ xmlDocumentRequestStr);
					// parentPanel.lastRequestMessage(
					// xmlDocumentRequestStr);
					response = QueryRequestClient
						.sendQueryRequestREST(xmlDocumentRequestStr);
					System.out
						.println("Generated XML document response: "
							+ response);
	...
	QueryConceptTreePanel panel = new QueryConceptTreePanel("Group "
	...
	jScrollPane4.setBounds(5, 35, width - 120, height - 65);
	// jPanel1.setPreferredSize(new Dimension(dataModel.lastLabelPosition(),
	// height-85));
	jScrollPane4.setViewportView(jPanel1);

	jOptionsScrollPane.setBounds(width - 112, 35, 108, height - 65);
	jOptionsScrollPane.setViewportView(jOptionsPanel);

	jRunQueryButton.setBounds(5, height - 24, width - 170, 23);
	// jNameTextField.setBounds(5, 4, width-121, 26);
	jNameLabel.setBounds(6, 6, width - 126, 23);
	// jQueryNamePanel.setBounds(5, 4, width-121, 26);
	jClearGroupsButton.setBounds(width - 116, 6, 115, 23);
	...
	protected Transferable createTransferable(JComponent c) {
		...
		if (jPanel1.getComponent(i) instanceof QueryConceptTreePanel) {
		    QueryConceptTreePanel panel = (QueryConceptTreePanel) jPanel1
			    .getComponent(i);// getTreePanel(i);
