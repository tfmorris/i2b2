...
// edu.harvard.i2b2.crcxmljaxb.datavo.vdo.GetChildrenType;
...
import edu.harvard.i2b2.query.ui.QueryConceptTreePanel;
import edu.harvard.i2b2.query.ui.QueryConstrains;

public class QueryConceptTreeNodeData implements QueryConstrains {
	...
	private void updateNodeMetaDataXML()
	{        	
		try {  
			...
			org.jdom.Element elementMsgBody = tableDoc.getRootElement().getChild("message_body"); //, Namespace.getNamespace("http://www.i2b2.org/xsd/cell/ont/1.1/"));
			org.jdom.Element tableXml = elementMsgBody.getChild("concepts", Namespace.getNamespace("http://www.i2b2.org/xsd/cell/ont/1.1/"));
			...
			for (Iterator itr=conceptChildren.iterator(); itr.hasNext(); ) {
				...
				if((metadataAttribs != null) && (valuedataAttribs != null)) {
					...
					if(dataTypeElement!=null && dataTypeElement.getTextTrim().equalsIgnoreCase("Enum")) {
						...
						log.debug("Got vals: "+this.valuePropertyData().enumValues.size());
					}
					...
					if(unitElement!=null) {
						for(int i=0; i<unitElement.getChildren().size(); i++) {
							...
							if(element.getName().equalsIgnoreCase("NormalUnits")) {
								...
								this.valuePropertyData().units.add(unitString);
							}
						}
						log.debug("Got vals: "+this.valuePropertyData().enumValues.size());
					}
	 ...
	 * 
	 * //String content = rawContent.substring(rawContent.indexOf("<patientData>")+13, //
	 ...
	 * sNodesXMLRepresentation += "\t<lookupdb>" + lookupdb() + "</lookupdb>\r\n";
	 * sNodesXMLRepresentation += "\t<lookuptable>" + lookuptable() + "</lookuptable>\r\n";
	 * sNodesXMLRepresentation += "\t<selectservice>" + selectservice() + "</selectservice>\r\n";
	 * sNodesXMLRepresentation += "\r\n\t</" + sIndividualConceptTag + ">\r\n";
