...
import edu.harvard.i2b2.explorer.data.TimelineFactory;
...
//			log.debug("Sresult: "+ soapResult.toString());
	...
	public static String getlldString(ArrayList<TimelineRow> tlrows, String patientRefId, 
		...
		try {
			...
			String xmlStr = pdoFactory.requestXmlMessage(items, pid, new Integer(minPatient), new Integer(maxPatient), false);
		...
		try {
			...
			for(int j=0; j<factSets.size(); j++) {
				...
				if (observationFactSet != null) { 
					log.debug("Total fact: "+observationFactSet.getObservationFact().size()
			...
			for(int i=0; i<patientDimensionSet.getPatientDimension().size();i++) {
				...
				log.debug("PatientNum: " + patientType.getPatientNum());
