...
//import edu.harvard.i2b2.query.QueryC;
//import edu.harvard.i2b2.query.QueryListNamesClient;
//import edu.harvard.i2b2.query.QueryMasterData;
//import edu.harvard.i2b2.query.QueryPreviousRunsPanel;
/*import edu.harvard.i2b2.querytool.datavo.DTOFactory;
import edu.harvard.i2b2.querytool.datavo.JAXBUtil;
import edu.harvard.i2b2.querytool.datavo.i2b2.ApplicationType;
import edu.harvard.i2b2.querytool.datavo.i2b2.BodyType;
import edu.harvard.i2b2.querytool.datavo.i2b2.CommonMessageHeader;
import edu.harvard.i2b2.querytool.datavo.i2b2.FacilityType;
import edu.harvard.i2b2.querytool.datavo.i2b2.MessageControlIdType;
import edu.harvard.i2b2.querytool.datavo.i2b2.MessageTypeType;
import edu.harvard.i2b2.querytool.datavo.i2b2.ProcessingIdType;
import edu.harvard.i2b2.querytool.datavo.i2b2.RequestMessageType;
import edu.harvard.i2b2.querytool.datavo.i2b2.ResponseMessageType;
import edu.harvard.i2b2.querytool.datavo.i2b2.SecurityType;
import edu.harvard.i2b2.querytool.datavo.i2b2.CommonMessageHeader.MessageHeader;
import edu.harvard.i2b2.querytool.datavo.i2b2.RequestMessageType.RequestHeader;
import edu.harvard.i2b2.querytool.datavo.querytool.HeaderType;
import edu.harvard.i2b2.querytool.datavo.querytool.MasterResponseType;
import edu.harvard.i2b2.querytool.datavo.querytool.QueryMasterType;
import edu.harvard.i2b2.querytool.datavo.querytool.QueryProcessorJAXBUnWrapHelper;
import edu.harvard.i2b2.querytool.datavo.querytool.QueryToolDTOFactory;
//import edu.harvard.i2b2.querytool.datavo.querytool.QuerySetType;
//import edu.harvard.i2b2.querytool.datavo.querytool.QueryType;
import edu.harvard.i2b2.querytool.datavo.querytool.RequestTypeType;
//import edu.harvard.i2b2.querytool.datavo.querytool.ResultSetType;
//import edu.harvard.i2b2.querytool.datavo.querytool.ResultType;
import edu.harvard.i2b2.querytool.datavo.querytool.UserRequestType;
import edu.harvard.i2b2.querytool.datavo.querytool.UserType;*/
//import edu.harvard.i2b2.querytool.datavo.querytool.ResultType.MasterQuery;
...
public class CRCNavigator extends ApplicationWindow {
	...
	// service calls
	// System.setProperty("ExplorerConfigurationXML",ssFakeApplicationConfigurationXML);
	...
	protected MessageHeaderType getMessageHeader() {
		/*CommonMessageHeader.MessageHeader messageHeader = new CommonMessageHeader.MessageHeader();
	...
	private String writeContentQueryXML() {
		...
		headerType.setRequestType(RequestTypeType.CRC_QRY_GET_QUERY_MASTER_LIST_FROM_USER_ID);
		...
		BodyType bodyType = new QueryToolDTOFactory().buildBodyType(headerType, userRequestType, null);
		...
		StringWriter strWriter = new StringWriter();
		try {
			jaxbUtil.requestMarshaller(requestMessageType, strWriter);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	private void loadPreviousQueries() {
		System.out.println(" Logged in, loading previous queries for: "+System.getProperty("user"));
		...
		try {
			JAXBUtil jaxbUtil = new JAXBUtil();
			ResponseMessageType messageType = jaxbUtil.unMashallResponseMessageTypeFromString(responseStr);
			
			BodyType bt = messageType.getMessageBody();
			MasterResponseType masterResponseType = (MasterResponseType) new QueryProcessorJAXBUnWrapHelper().getResponseType(bt);
	...
	protected void configureShell(Shell shell) {
		...
		shell.setText(System.getProperty("applicationName") + " CRC Navigator Version 2.0");
	...
	protected Control createContents(Composite parent) {
		...
		statusLabelPaintListener.setOvalColor(display.getSystemColor(SWT.COLOR_YELLOW));
		statusOvalLabel.setSize(20,20);
		statusOvalLabel.redraw();
			...
			menuItem.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent event) {
					...
					//if(menu.indexOf(selected) == 1) {
						...
						Composite selectedTab = (Composite) items[menu.indexOf(selected)];
						if((selectedTab.getChildren()[0]).getClass().getSimpleName().equalsIgnoreCase("ExplorerC")) {
							//ExplorerC qc = (ExplorerC)(selectedTab.getChildren()[0]);
							//qc.getAWTContainer().requestFocus();
							//qc.runTreePanel().reset();
						}
						else if((selectedTab.getChildren()[0]).getClass().getSimpleName().indexOf("Query")>=0) {
	...
	public void loginAction(boolean login) {
		...
		if (login) {
			//try to connect to tomcat server if not available show message and exit
			String webservice =getCRCNavigatorProperties();
			String msg="";
			//webservice="http://phsi2b2appdev.mgh.harvard.edu:8080/i2b2/services/Select";
						/*
			// try to connect to tomcat server if not available show message and logout
			try {
				
//				SelectService service = new SelectServiceLocator();
	//			java.net.URL endpoint = new java.net.URL(webservice);
		//		service.getSelect(endpoint);
				log.info(getNow() + " Start login");
	...
	//		} catch (MalformedURLException e) {
				//e.printStackTrace();
				log.error(e.getMessage());
				//MessageBox messageBox=new MessageBox(getApp().getShell(),SWT.OK);
		...
		if (userInfoBean == null) {
			log.info(getNow() + " Login fail or cancel");
			// System.out.println(getNow()+ " Login fail or cancel");

			// remove all tab items
			int numtabs = tabFolder.getItemCount();
			if (numtabs > 0) {
				TabItem[] tabItemList = tabFolder.getItems();
				for (int i = 0; i < tabItemList.length; i++) {
					tabItemList[i].dispose();
				}
			}
			// remove menu items
			int numitems = menu.getItemCount();
			if (numitems > 0) {
				MenuItem[] menuItemList = menu.getItems();
				for (int i = 0; i < menuItemList.length; i++) {
					menuItemList[i].dispose();
				}
			}
		...
		*/
			...
			// Mode"};
			/* OLD?????
			String[] userModes = userInfoBean.getUserModes();
			String[] userModeValues = userInfoBean.getUserModeValues();
			String userDefaultMode = userInfoBean.getDefaultUserMode();
			String environment = userInfoBean.getEnvironment();
			if (!userInfoBean.getHelpURL().equals(""))
				helpURL=userInfoBean.getHelpURL();

			// add menu items
			addMenuItems(menu, userModes, userModeValues);
			...
			// set bannerC background color from config file here
			// if environment not specified defaults to development (dark gray)
			APP_CURRENT = environment;
			if (APP_CURRENT.equals(APP_PROD)) {
			...
			// System.out.println("userMode[0]=" +userModeValues[0]);
			if (defaultMode.equals("")) {
				defaultMode = userModeValues[0];
			}
			defaultMode = defaultMode + " Mode";
			titleToolItem.setText(defaultMode);

			// set button text to logout
			loginButton.setText(BUTTON_TEXT_LOGOUT);
