...
public class mainPanel extends Panel{
    ...
    private picPanel logoPicPanel;/* Added by Partha - 10/15 this is the lifelines logo*/
    private picPanel clogPicPanel;/*Added by Partha - 11/12 this is the copyright label*/
    private picPanel hlogPicPanel;/*Added by Partha - 11/12 this is the hcil logo*/
    ...
    public void init() {
        ...
        //System.out.println("theTimeLinePanel.height = " + theTimeLinePanel.height);
        //System.out.println("theTimeLinePanel.dataheight = " + theTimeLinePanel.getTimeLinePanelDataheight());
        //tlpScroll = new Scrollbar(Scrollbar.VERTICAL,0,height-24,0,13209);
        //tlpScroll = new Scrollbar(Scrollbar.VERTICAL,0,height-24,0,theTimeLinePanel.getDrawnHeight());
    ...
    public mainPanel(int width, int height,record theApplet,MyDate today) {
        ...
        //*add(clogPicPanel);
        clogPicPanel.setBounds(width-260,0,256,19);/*changed 7 to 0 to save space */
        ...
        if(!doublePanels) {
            theTimeLinePanel = new timeLinePanel(width-22 - thumbWidth, height - 62,theApplet,today);
            //theTimeLinePanel = new timeLinePanel(width-22 - thumbWidth+200, height - 62,theApplet,today);
            add(theTimeLinePanel);
            theTimeLinePanel.setBounds(10, 36, width-22- thumbWidth, height - 62);
            //theTimeLinePanel.setBounds(10, 36, width-22- thumbWidth+200, height - 62);
        }

        // snm - i think the problem is here
        // the problem is that the paint method theTimeLinePanel.paint() has not run yet
        // but to run it needs a graphics handle
        
        //System.out.println("theTimeLinePanel.dataheight = " + theTimeLinePanel.getTimeLinePanelDataheight());
        tlpScroll = new Scrollbar(Scrollbar.VERTICAL,0,height-74,0,23209);
        //tlpScroll = new Scrollbar(Scrollbar.VERTICAL,0,height-24,0,(height-24)*20);
        //tlpScroll = new Scrollbar(Scrollbar.VERTICAL,0,height-24,0,theTimeLinePanel.getDrawnHeight());
        ...
        if(doublePanels) {
            theTimeLinePanel = new timeLinePanel(width/2-22 - thumbWidth, height - 103,theApplet,today);
            add(theTimeLinePanel);
            theTimeLinePanel.setBounds(10, 43, width/2-22- thumbWidth, height - 103);

            anotherTimeLinePanel = new timeLinePanel(width/2-22 - thumbWidth, height - 103,theApplet,today);
            add(anotherTimeLinePanel);
            anotherTimeLinePanel.setBounds(width/2-22- thumbWidth + 10 + 30, 43, width/2-22- thumbWidth, height - 103);
        }
        ...
        theWeekSlider.setBounds(sliderOffset + 10 - thumbWidth, height-59, sliderWidth, 18);

        theMonthSlider= new monthSlider(sliderWidth,18,today);
        theMonthSlider.setBounds(sliderOffset + 10 - thumbWidth, height-41, sliderWidth, 18);

        theYearSlider= new yearSlider(sliderWidth,18,today);
        add(theYearSlider);
        theYearSlider.setBounds(sliderOffset + 10 - thumbWidth, height-23, sliderWidth, 18);
        preferButton = new Button("Zoom"); // for zoom button
        }

    public static void setApplet(newApplet inApplet) {
        theTimeLinePanel.setApplet(inApplet);
        theYearSlider.setApplet(inApplet);
        theMonthSlider.setApplet(inApplet);
        theWeekSlider.setApplet(inApplet);
    }
	...
	void panel1_MouseClick(java.awt.event.MouseEvent event)
	 ...
	 void panel3_MouseClick(java.awt.event.MouseEvent event)
        {
                this.theApplet.showDocument("http://www.cs.umd.edu/hcil/Research/1997/patientrecord.html");
        }
