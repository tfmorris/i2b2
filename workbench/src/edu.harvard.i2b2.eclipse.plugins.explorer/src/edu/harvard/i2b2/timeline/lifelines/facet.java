...
public class facet extends Panel{
    ...
    public Color backgroundColor; // added 1/12/98 dan: also added to constructor and in draw method
    public boolean enabled; // = record.openFacets; // 1/6/98 
    private int enabled_count; 
    private int remove_count;
    private Rectangle currentRect;
    public Rectangle currentRect() {return currentRect;}
    
    public facet(String token, Hashtable facetList,Color backgroundColor, boolean openFacet) { // last part
    // of argumemt list added 1/12/98 by dan, note Color is used not MyColor (as in loadRecord)
        title = token;
    ...
    /**
     * Store all the events to a vector so that events can be later laid out in different ways
     */
	      ...
	      for (int v=0; v<aggregates.size(); v++)    {
	        ...
	        if ((record.searchoption_timeline[1]) ? addThis.selected:true)  {
	            if (facetLines == null ) {
	                ...
	                if (record.option[4]) temp.setLabelString(addThis.getLabelString());   
	            }
	            else {
	                if (record.option[1] || record.option[2]) {  // compact the timelines to the minimum rows 
	                    int size = facetLines.size();   // need to set this at the beginning because the value can change inside the for loop
	                    int i=0;
	                    forloop: for (i=0; i < size; i++)   {
	                        temp = (facetLine)(facetLines.elementAt(i));
	                        if (temp.fits(addThis))  { // if current event fits the row, it will be added 
	                            if(record.comments) { // if statement: 1/6/98
	                                addThis.getStartdate().print();
	                    ...
	                    if (i == size)  {   // start a new row
	                        temp = new facetLine(title, backgroundColor);
	                        if(record.comments) {   // if statement added 1/6/98
	                            addThis.getStartdate().print();
	                            System.out.println("new timeline");
				            }
	                        facetLines.addElement(temp);
	                    ...
	                    forloop: for (i=0; i < size; i++)   {
	                        temp = (facetLine)(facetLines.elementAt(i));
	                        if (!temp.getLabelString().equals("") && temp.getLabelString().equals(addThis.getLabelString()))   {
	                            if (temp.fits(addThis)) {
	                                ...
	                                if (i != size-1)       // not at the last row yet
	                                    facetLines.insertElementAt(temp, i+1);
	                                else
	                                    facetLines.addElement(temp);
	                            }
	                            break forloop;
	                ...
	                else if (record.option[0])   {   // only see whether the event fits the current row or not      
	                    if(record.comments) 
	                        System.out.println("normal"); 
	                    temp = (facetLine)(facetLines.lastElement());
	                    if(!temp.fits(addThis)) {
	                        temp = new facetLine(title, backgroundColor); // added back in 12/23/97 removed for compact
	                        facetLines.addElement(temp);  // start a new row
	                    }
	                }
	            }
	            temp.addEventObject(addThis); // add the event to the current row
     ...
     */
    public void draw(int currentY, timeLinePanel displayPanel, boolean relabeling, boolean slide, boolean stream, boolean data, boolean label) {       
      ...
      if(enabled) {
              if (record.timeoption[1] && slide)   {  
                    ...
                    for (int i=0; i<orig_aggregates.size(); i++) {
                        ...
                        if (! displayPanel.offScale(thisagg))  {   // Otherwise, don't consider 
                            //thisagg.setStartdate(displayPanel.scaleMin((genRecord)thisagg));
                            //thisagg.setEnddate(displayPanel.scaleMax((genRecord)thisagg));
                ...
                if (record.searchoption_timeline[1] && record.theTabPanel.theTimeLinePanel.search)  {
                    this.layout();
                    //System.out.println("re layout");
                }
                ...
                if (facetLines == null)  {             
                    // 3/10/98 need to remove these codes after fitlabel function, height is determined by fitlabel()
	                currentRect = new Rectangle(0,currentY,displayPanel.getFullWidth(),getHeight()+12);
                    ...
                    if (relabeling || enabled_count==0)  { // || (record.searchoption_timeline[1] && record.theTabPanel.theTimeLinePanel.search))     // 3/28/98
		                for(int i=0;i<facetLines.size();i++) {
		                    temp = (facetLine)(facetLines.elementAt(i));   
		                    // don't consider cross facetlines
		                    temp.resetlabel();
		                    
		                    temp.fitlbl = temp.fitlabel(currentY, displayPanel, false);   
		                }
	                ...
	                // 3/10/98 need to remove these codes after fitlabel function, height is determined by fitlabel()
	                currentRect = new Rectangle(0,currentY,displayPanel.getFullWidth(),getHeight()+5);
                    // +5 added to fill in the gap, but not yet exact dan 1/12/98
                    g.setColor(backgroundColor);
                    g.fillRect(currentRect.x,currentRect.y,currentRect.width,currentRect.height);
                    g.setColor(Color.black);
                    ...
                    g.fillPolygon(xCoordinates,yCoordinates,3); 
                    g.setFont(new Font("TimesRoman", Font.PLAIN, 14));
                    g.drawString(title,15,currentY + 12); // 1/13/98 by dan... show labels even when
                    // open, loses some vertical space, maybe use abbreviation here eventually

                    for(int i=0;i<facetLines.size();i++) {
                        
		                temp = (facetLine)(facetLines.elementAt(i));
		                
		                if ((record.lbloption[1] || record.lbloption[2]) && !temp.fitlbl) {
		                    // later, adjust timeline
		                    temp.setSavedLabelXY();  
		                    if (data) temp.drawData(currentY, displayPanel, false);
		                    if (label) temp.drawLabel(currentY, displayPanel, stream);
		        ...
		        if (facetLines != null) {   // might be because of sliding
		            for(int i=0;i<facetLines.size();i++) {
		                temp = (facetLine)(facetLines.elementAt(i));
		                if (data) temp.drawData(currentY,displayPanel,true);
   ...
   /**
    * Returns the event if the point (x,y) falls inside the region of the event or its corrsponding label
    * Otherwise, return null.
    */
   public genRecord inRegion(int x, int y, boolean data, boolean label, int distance) {
        ...
        for(int i=0;i<facetLines.size();i++) {
            ...
            for (int j=0; j<tempFacetLine.getAggregates().size(); j++)   {
                ...
                for (int k=0; k<tempAgg.getAllRecords().size(); k++)   {
                    ...
                    if ((data && dataRect.contains(x,y)) || (label && labelRect.contains(x,y)))
                        return tempStory;
                    else  {
                        int selectedX, selectedY;
                        if (Math.abs(dataRect.x - x) <= Math.abs(dataRect.x + dataRect.width - x))
                            selectedX = dataRect.x;
                        else
                            selectedX = dataRect.x + dataRect.width;
                        if (Math.abs(dataRect.y - y) <= Math.abs(dataRect.y + dataRect.height - y))
                            selectedY = dataRect.y;
                        else
                            selectedY = dataRect.y + dataRect.height;
                        double dis = Math.sqrt((selectedX-x)*(selectedX-x) + (selectedY-y)*(selectedY-y));
                        if (dis < minDis)   {
                            minDis = dis;
  ...
  public Vector rubber_band(int rubber_startX, int rubber_startY, int rubber_endX, int rubber_endY) {
        ...
        for(int i=0;i<facetLines.size();i++) {
            temp = (facetLine)(facetLines.elementAt(i));
            substreamlist = temp.rubber_band(rubber_startX, rubber_startY, rubber_endX, rubber_endY);
