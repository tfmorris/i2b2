...
public class aggregate extends genRecord {
    ...
    private Hashtable genList; // hashtable allows dynamic sizing; 7/23/98 - could contain aggregates as well
    private int height;
    private int width;
    private int counter; // since numberOfEvents is read in from the file...
    private storyRecord summary;   // 3/10/98 and then 7/25/98 change to storyRecord
    private Hashtable rectIndexList; // needed for now because storyRecord keeps a current index called rectp
    // also this is allocated ("newed") only in draw since the values (but not likely the number, though this
    // would be determined after all adds (not in constructor), so don't do for now..)
    ...
    public MyDate getDateMax()
    {
        ...
        MyDate dateMax = new MyDate(getStartdate().getMonth(),
        		getStartdate().getDay(),
    ...
    public int getRectWidth()
    {
        ...
        for(int i=0;i < genList.size();i++) {
            if(rectWidth < ((genRecord)(genList.get(new Integer(i)))).getRectWidth() )
                rectWidth = ((genRecord)(genList.get(new Integer(i)))).getRectWidth();
        }
        return rectWidth;
    }
    ...
    public void redraw() { 
        ...
        for(int k = 0; k < genList.size(); k++)     {
            ...
            r.width = -1;
            r.height = -1;
            System.out.println("set rect to: "+aGenRecord.getBarArea().x+","+aGenRecord.getBarArea().y);*/
        }
    }
    ...
    public void addGen(genRecord gen)  {
        ...
        if (genList.size() > 1 && gen.getStartdate().equals(((genRecord)genList.get(new Integer(counter-2))).getStartdate()))
            cum += ((genRecord)genList.get(new Integer(counter-2))).getRectWidth() + 1;
        else  {           
            if (height < cum)  
                height = cum;
            cum=0;
        ...
        if(start_date == null)
	       ...
	       start_date = new MyDate(gen.getStartdate().getMonth(),
	    		   gen.getStartdate().getDay(),
	    		   gen.getStartdate().getYear(),
	    		   gen.getStartdate().getHour(),
	    		   gen.getStartdate().getMin());

        if(end_date == null)
           end_date = (gen.getEnddate()).copy();
        else if (gen.getEnddate().after(end_date))
           end_date = new MyDate(gen.getEnddate().getMonth(),
        		   gen.getEnddate().getDay(),
        		   gen.getEnddate().getYear(),
        		   gen.getEnddate().getHour(),
        		   gen.getEnddate().getMin());
    ...
    public void drawLabel(int currentY, timeLinePanel displayArea, boolean label, boolean summaryrecord, boolean stream) { 
        ...
        if (!all_labelled && (summary!=null) && (summary.getLabelX()>=0 || summary.getSaveLabelX()>=0))  
               summary.drawLabel(currentY, displayArea, summaryoption[0], true, stream);
               
        for(int k = 0; k < genList.size(); k++)     {
            aGenRecord = (genRecord)(genList.get(new Integer(k)));
            
            if (!all_labelled && (summary!=null) && (summary.getLabelX()>=0 || summary.getSaveLabelX()>=0))  
                    aGenRecord.drawLabel(currentY, displayArea, !summaryoption[0], false, stream);                    
            else  
                aGenRecord.drawLabel(currentY, displayArea, true, false, stream);
        }
    }
    ...
    public void drawData(int currentY, timeLinePanel displayArea, boolean silhouette, boolean timeline, boolean summaryrecord) { 
        ...
        if (!all_labelled && (summary!=null))  
               summary.drawData(currentY, displayArea, silhouette, summaryoption[1], true);
               
        for(int k = 0; k < genList.size(); k++)     {
            aGenRecord = (genRecord)(genList.get(new Integer(k)));
         
            if (!all_labelled && (summary!=null))
                aGenRecord.drawData(currentY, displayArea, silhouette, !summaryoption[1], false);
            else   {
                aGenRecord.drawData(currentY, displayArea, silhouette, true, false);
                 
                // 10/26/98 Julia, handle parallel aggregates
		    genRecord nextGenRecord = aGenRecord; // just a dummy initialization
               
                if(k < (genList.size() - 1) )  
                    nextGenRecord = (genRecord)(genList.get(new Integer(k + 1)));
                
                if ((nextGenRecord.getStartdate()).equals(aGenRecord.getStartdate()))   
                //    !aGenRecord.getCause().equals("nolabel") && !nextGenRecord.getCause().equals
                //    ("nolabel")) 
    ...
    public boolean contains(String searchString) {
        ...
        for(int i=0;i < genList.size();i++) {
            if(((genRecord)(genList.get(new Integer(i)))).getCause().equals(searchString))
               return true;
        }
        return false;
    }
     ...
     */ 
    public Vector rubber_band(int rubber_startX, int rubber_startY, int rubber_endX, int rubber_endY) {
        ...
        for (int i=0; i < genList.size(); i++)  {
            ...
            if (temp.getType().equals("aggregate"))  {
                subrubberlist = temp.rubber_band(rubber_startX, rubber_startY, rubber_endX, rubber_endY);
                for (int j=0; j < subrubberlist.size(); j++)
                    rubberlist.addElement(subrubberlist.elementAt(j));
            }
            else    {
                storytemp = (storyRecord)temp.intersects(rubber_startX, rubber_startY, rubber_endX, rubber_endY);
                if (storytemp != null)
    ...
    public Vector rubber_band(int centerX, int centerY, int radius) {
        ...
        for (int i=0; i < genList.size(); i++)  {
            ...
            if (temp.getType().equals("aggregate"))  {
                ...
                storytemp = ((storyRecord)temp).intersects(centerX, centerY, radius);
                if (storytemp != null)
                    rubberlist.addElement(storytemp);
            }
        }
    ...
    public void setInitFlags(MyRectangle rects[] ) {
        ...
        for(int i=0;i<rectIndexList.size();i++) {
            rects[((Integer)(rectIndexList.get(new Integer(i)))).intValue()].setInitFlag();
        }
    ...
    public void unSetInitFlags(MyRectangle rects[] ) {
        ...
        for(int i=0;i<rectIndexList.size();i++) {
            rects[((Integer)(rectIndexList.get(new Integer(i)))).intValue()].unSetInitFlag();
        }
    }
     ...
     */
    public void setXOverlap(conflictResolver xOverlap1_, conflictResolver xOverlap2_) {
    ...
    public Vector getAllRecords()  {
        ...
        for (int i=0; i<genList.size(); i++)    {
            if (! ((genRecord)genList.get(new Integer(i))).getType().equals("aggregate"))
                allRecord.addElement((storyRecord)genList.get(new Integer(i)));
            else  {
                Vector tempRecord = ((aggregate)genList.get(new Integer(i))).getAllRecords();
    ...
    public boolean fitlabel(int currentY, timeLinePanel displayArea, boolean backtrack, int height) {
        ...
        while (mark < genList.size() && mark >= 0) {
          ...
          // do a cast to a labelless class of genRecord to get paralell aggregates
          // 3/28/98 Don't label the ones that are not selected for search
          if ((aGenRecord.getSelected() && record.searchoption_label[1]) || record.searchoption_label[0])  {
            fitlabel = aGenRecord.fitlabel(currentY, displayArea, backtrack, height);

            if ((lbloption[1]||lbloption[2]) && ! fitlabel) {  // add if the before is aggregate, ignore it
                mark--;
                num--;
                ...
                backtrack = false;
                num++;     
                if (lbloption[0] && !fitlabel) all_labelled = false;  
            }
          }
          else
            mark++;   
          // save the label configuration to maxmize the number of events get labelled
          if (num > numLabel) {
                numLabel = num;
	    ...
	    genRecord lastRecord = (genRecord)genList.get(new Integer(genList.size()-1));
	    if (lbloption[2] && lastRecord.getXOverlap2() != null)   // if lastRecord is an aggregate, getXOverlap2 will get the xOverlap2 of the last event in the aggregate
	        below = (! lastRecord.getXOverlap2().isEmpty());

	    if (lbloption[1]||lbloption[2]) all_labelled = fitlabel;  // all the previous items must have to be labelled, so we only need to check the last one
	    if (!all_labelled && summaryoption[0] && (summary!=null)) {   // display summary label for the first event record
	      ...
	      fitlabel = summary.fitlabel(currentY, displayArea, backtrack, height);
    ...
    public void select(String searchString) {
        ...
        selected = false;   
        if (summary != null)  summary.selected = false;
        for(int i=0;i<genList.size();i++) {
	        try {

	            RegExp reg = new RegExp(searchString); // gnu public licence regexp package (presently
