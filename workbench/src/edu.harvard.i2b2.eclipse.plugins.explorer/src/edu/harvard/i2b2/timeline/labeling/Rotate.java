...
public class Rotate extends Applet {
    ...
    public Rotate (String tx, Point p, Font font, Color backgroundColor, int degree, record applet) {
        Frotate rot;
...
// Begin - Included from OffScr.include
	...
	/*
        if(tx != null) {
            ...
            <a href=ftext.java.html>ftext</a> ps = new <a href=ftext.java.html>ftext</a>();
            ps.setText(tx);
            ps.setApplet(this);
            ps.setFont(tx_font);
            ps.setBackground(app_bg_color);
	...
	*/
        ...
        img_bordercolor = GetParmToColor(   "ImgBorderColor",
                                                   Color.black);
        ...
        for(i=0;i<num_maps;i++) {
            ...
            if(map_audio_file[i] != null) {
                ...
                if(map_clip[i] == null) {
                    System.out.println(
                        "Error getting " + map_audio_file[i] + ". " +
                        "Make sure the file exists and is an audio file");
                }
            }    
    ...
    public void paint(Graphics g) {
        ...
        if(tx_osi != null) {
            tx_x_coord = calcULXcoord(tx_horizcenter, size().width,
                            tx_width, tx_xoffset);
            tx_y_coord = calcULYcoord(tx_vertcenter, size().height,
                                tx_height, tx_yoffset);
    ...
    public boolean mouseDown(Event evt, int x, int y) {
        ...
        for(i=0;i<num_maps;i++) {
            if(x > map_x1[i] && x < map_x2[i] &&
               y > map_y1[i] && y < map_y2[i]) {
                if(map_url[i] != null) {
                    if(testmode) {
                        getAppletContext().showStatus("Loading URL: " +
                                    map_url[i].toString());
        ...
        if(x > tx_x_coord && x < tx_x_coord + tx_width &&
           y > tx_y_coord && y < tx_y_coord + tx_height) {
            if(tx_url != null) {
        ...
        if(x > img_x_coord && x < img_x_coord + img_width &&
           y > img_y_coord && y < img_y_coord + img_height) {
            if(img_url != null) {
    ...
    //
    protected void drawText(Graphics g, String txt, int x, int y,
                                            Color c, Font f) {
        if(txt != null) {
            g.setColor(c);
            g.setFont(f);
    ...
    //
    protected void drawUnderline(Graphics g,
                           int x,
                           int y,
                           int w,
                           Color c) {
    ...
    //
    protected int calcULXcoord(boolean hcent, int appletwidth,
                                int objectwidth, int xoffset) {
        int x;
            ...
            if(tile & (bgimageXoffset != 0 || bgimageYoffset != 0)) {
                for(i=-bgimage_height;i<app_height;i+=bgimage_height) {
                    for(j=-bgimage_width;j<app_width;j+=bgimage_width) {
                        bg_g.drawImage(bgimage, j+bgimageYoffset,
                                        i+bgimageXoffset, this);
                    }
                }
                ...
                bg_g.drawImage(bgimage, bgimageXoffset,
                                        bgimageYoffset, this);
            }
        }
    ...
    public void doStandardFilters() {
        ...
        if(img_osi != null && img_filter_str != null) {
            img_pixels = invoke(null, img_filter_str, img_pixels,
                            img_width, img_height, false, app_bg_color,
                            false);
            img_width  = getWidth();
        ...
        if(img_osi != null) {
            img_osi = createImage(new MemoryImageSource(img_width,
                            img_height, ColorModel.getRGBdefault(),
                            img_pixels, 0, img_width));
        }

        //========= INVOKE FILTERS ==============
        if(tx != null && tx_filter_str != null) {
            if(tx_bg_color == null) {
                tx_pixels = invoke(tx, tx_filter_str, tx_pixels,
                            tx_width, tx_height, true, app_bg_color,
                            tx_underline);
            } else {
                tx_pixels = invoke(tx, tx_filter_str, tx_pixels,
                            tx_width, tx_height, false, tx_bg_color,
                            tx_underline);
            }
            tx_width  = getWidth();
            tx_height = getHeight();
        ...
        //========= CREATE IMAGE FROM PIXELS ==============
        if(tx != null) {
            tx_osi = createImage(new MemoryImageSource(tx_width,
                            tx_height, ColorModel.getRGBdefault(),
                            tx_pixels, 0, tx_width));
        }
    }
...
// Begin - Inlucded from Filter.include
    ...
    public int[] invoke(String tx, String f_str, 
                        int[] p1, int w, int h,
                        boolean maketransparent, Color bgcolor,
                        boolean underline) {
	return invoke(tx, f_str, p1, w, h, maketransparent, 
			bgcolor, underline, getFont());
    }

    public int[] invoke(String tx, String f_str, 
                        int[] p1, int w, int h,
                        boolean maketransparent, Color bgcolor,
                        boolean underline, Font font) {
        ...
        if(f_str != null) {
            ...
            for(i=0;i<num_elements;i++) {
                try {
                    ...
                    System.err.println("Couldn't instantiate class " + filter_strings[i]);
                } catch (IllegalAccessException e) {
                    System.err.println("Couldn't access class " + filter_strings[i]);
                }
            }
        }
        return p1;
