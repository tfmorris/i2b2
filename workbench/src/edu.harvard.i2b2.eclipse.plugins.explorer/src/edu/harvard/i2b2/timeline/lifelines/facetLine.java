...
public class facetLine {
  ...
  public void addEventObject(genRecord addThis) {
        ...
        if(((genRecord)(addThis.getGenList().get(new Integer(0)))).getInputLine().indexOf("Value")>=0) {
        		tmp = 22;
        }
  ...
  public int getHeight() {
      return height + 10; // this is an approximation of fontTextHeight which depends on the displayPanel...
  }

  /**
   * Label all the events in the facetLine
   */
  public boolean fitlabel(int currentY, timeLinePanel displayArea, boolean backtrack) {
	   ...
	   while (mark < aggregates.size() && mark >= 0) {
	     ...
	     if ((temp.selected && record.searchoption_label[1]) || record.searchoption_label[0])  {
	        fit = temp.fitlabel(currentY, displayArea, backtrack, getHeight());

	        if ((lbloption[1]||lbloption[2]) && ! fit) {   // the labels of the aggregate cannot fit and need to backtrack the previous aggregate
	            mark--;
	            num--;
	            if (mark < 0)   // cannot fit the whole line
		            ...
		            for (int i=mark+1; i<aggregates.size(); i++)
		                ((genRecord)(aggregates.elementAt(i))).resetlabel();
	            }
	        }
	        else  {
	            //temp.draw(currentY, displayArea);   // probably save for future
	            mark++;
	            backtrack = false;
	            num++;
	        }  
  	     ...
  	     if (num > numLabel) {
                ...
                for (int i=0; i<mark; i++)    {
                    ...
                    for (int j=0; j<aGenRecord.getGenList().size(); j++)   {
                        aRecord = (genRecord)(aGenRecord.getGenList().get(new Integer(j)));
                        aRecord.saveLabelXY();
                    }
                }
         }
  	   }
	   // 3/28/98 label the aggregates that have never been labeled because the previous ones cannot fit
  ...
  /**
   * Check whether all the aggregates get labeled
   */
  private boolean agg_all_label()   {
    boolean all_label=true;
    for (int i=0; i<aggregates.size(); i++)  {
        if  (!((aggregate)(aggregates.elementAt(i))).labelonce)
            all_label=false;
    }
    return all_label;
  }
  ...
  public void resetlabel() {
	    ...
	    for (int i=0; i<aggregates.size(); i++) {
           ...
           temp.numLabel = 0;
           temp.below=false; temp.all_labelled=true; temp.selected=true; temp.labelonce=false;
           temp.resetlabel();
   ...
   */
  public void drawLabel (int currentY,timeLinePanel displayArea, boolean stream) {    
       FontMetrics fm = record.theTabPanel.theTimeLinePanel.fontMetrics1;
  	   aggregate temp;
   ...
   */
  public void drawData (int currentY,timeLinePanel displayArea, boolean silhouette) {    
  	   aggregate temp;
  ...
  /**
   * Set the event name for the facetLine. This is used in the "group by event name" layout.
   */
            ...
            for (int j=0; j<aAgg.getGenList().size(); j++)  {
                ...
                if (!aStory.getCause().equals(" "))
                    labels.addElement(new LiteLabel(aStory.getCause(),
                                         new Point(aStory.startX, aStory.startY),
                                         0,
                                         record.theTabPanel.theTimeLinePanel.font1,
                                         Color.black,
                                         backgroundColor, degree, record.theTabPanel.theTimeLinePanel.thisApplet));
            }
        }
   }
}
