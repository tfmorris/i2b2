...
public class Frotate extends ImgFilt {
	...
	newx = ((int)Math.round(((w) * cosine) - 
			((h) * sine)));
	newy = ((int)Math.round(((w) * sine) + 
			((h) * cosine)));
	biggestx = newx;
	biggesty = newy;
	smallesty = newy;

	newx = ((int)Math.round(((0) * cosine) - 
			((h) * sine)));
	newy = ((int)Math.round(((0) * sine) + 
			((h) * cosine)));
	if(newx > biggestx)
	    biggestx = newx;
	if(newy > biggesty)
    ...
    for(y=0;y<new_height;y++) {
        for(x=0;x<new_width;x++) {
                oldx = (int)Math.round((x * cosine) - ((y+smallesty) * sine));
                oldy = (int)Math.round((x * sine) + ((y+smallesty) * cosine));
