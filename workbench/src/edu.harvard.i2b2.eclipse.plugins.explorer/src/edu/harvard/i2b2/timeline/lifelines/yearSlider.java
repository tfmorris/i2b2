...
private int yearRange; // minutes in year, appropriately scaled.
    ...
    public yearSlider(int width, int height,MyDate today){
        ...
        level = 0;
        yearRange = (((int)Math.round((double)rangeWidth * 365 * 24 * 60 / diff))); // minutes in year, appropriately
        //scaled
        status = "enabled";
        talk(INIT);
    }
        ...
        switch(currentPos){
            case MINTHUMB:
                ...
                /* trackerVariable = x;
                 ...
                 + Integer.toString(x));
                 System.out.println(Integer.toString(validDateMin.getYear()) + "/" +Integer.toString(validDateMin.getMonth()
                 ...
                 if(minThumbPos + minOffset > (maxThumbPos - thumbWidth) ) break; // 3/16/98 dan to fix overlap
                 // problem on drag (see also below)

                 //if(rangeWidth < minOffset + yearRange) minOffset = rangeWidth - yearRange;
		 /*else*/ if(-minOffset > minClipWidth) minOffset = -minClipWidth; // 3/16/98 dan
                ...
                //    repaint(); // this had been removed...
                    ...
                    if(rangeWidth == yearRange)
                       record.theTabPanel.theMonthSlider.wakeup("enabled",validDateMin);
                           ...
                           record.theTabPanel.theMonthSlider.getStatus().equals("enabled"))
                       record.theTabPanel.theMonthSlider.wakeup("disabled",null);
                 }
                 break;

            case MAXTHUMB:
                 maxOffset = oldMaxValue - x;
                 //if(rangeWidth < maxOffset + yearRange) maxOffset = rangeWidth - yearRange;
		 /*else*/ if(-maxOffset > maxClipWidth) maxOffset = -maxClipWidth; // 3/16/98 dan
                 // to try to allow full dragging
                 if(maxOffset != 0){
		   //if(rangeWidth-maxOffset < yearRange) break; 3/16/98 ditto
		   if(maxThumbPos - maxOffset < minThumbPos + thumbWidth) break; // 3/16/98 dan to fix overlap bug
