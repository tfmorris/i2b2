...
public class PDORequestMessageFactory {
	...
	private PdoQryHeaderType buildHeaderType() { 
		...
		pdoHeader.setRequestType(PdoRequestTypeType.GET_OBSERVATIONFACT_BY_PRIMARY_KEY);
		return pdoHeader;
	}
	...
	public GetObservationFactByPrimaryKeyRequestType buildFactRequestTypeByPrimaryKey(int patientNum, 
		...
		try {
			...
			xmlCalendar = dataTypeFactory.newXMLGregorianCalendar(year, month, day, 0, 0, 0, 0, -5*60);
		}
	...
	public String  requestXmlMessage(int patientNum, int encounterNum, 
		...
		GetObservationFactByPrimaryKeyRequestType patientSetRequestType = 
			buildFactRequestTypeByPrimaryKey(patientNum, encounterNum, concept_cd, providerId,
					modifier_cd, start_date);
