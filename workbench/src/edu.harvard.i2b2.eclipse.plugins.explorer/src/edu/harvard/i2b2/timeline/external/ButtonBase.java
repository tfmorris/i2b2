    ...
    extends Canvas
{
    ...
    /**
     * Set the showFocus indicator to visual show when the mouse enters the Button
     * @param boolean f true = show the mouse enter; false = do not show the mouse enter
     * @see #getShowFocus
     */
     ...
     * If the mouse was pressed inside the button then post an event.
	...
	public boolean action(Event e, Object o)
    {
        if (notifyWhilePressed)
        {
            if (e.target == notifyTimer && true) //!symantec.beans.Beans.isDesignTime())
            {
                postEvent(new Event(this, Event.ACTION_EVENT, null));
                return true;
            }
