/*
 * @(#) $RCSfile: DefaultTableContext.java,v $ $Revision: 1.3 $ $Date: 2008/10/31 17:43:22 $ $Name: RELEASE_1_3_1_0001b $
 *
...
public class DefaultTableContext implements TableContext {
    ...
    static Vector defaultViewList = new Vector(); // an ordered list of built

    // in viewNames

    static Hashtable defaultViewIcons24 = new Hashtable(); // viewClass ->

    // 16x16icon

    static Hashtable defaultViewIcons16 = new Hashtable(); // viewClass ->
    // 24x24icon
    ...
    private Hashtable views = (Hashtable) defaultViews.clone(); // viewName ->

    // viewClass

    private Vector viewList = (Vector) defaultViewList.clone(); // an ordered
	...
	public void stateChanged(ChangeEvent e) {
		...
		setColumnMapSetOperator(((SetOperator) e.getSource())
			.getSetOperator());
	    }
	}
    };
	...
	public int compare(Object o1, Object o2) {
	    return o1 == o2 ? 0 : o1 == null ? -1 : o2 == null ? 1 : o1
		    .hashCode() < o2.hashCode() ? -1 : 1;
	}
     ...
     * 
     * @param f
     *            the frame to size and position
     * @param x
     *            the default x location for the frame
     * @param y
     *            the default y location for the frame
     * @param w
     *            the default width for the frame
     * @param h
	...
	if (ids != null) {
		...
		if (className != null) {
			...
			viewName = idx < 0 ? className : className
				.substring(idx + 1);
		    }
		    try {
			Class theClass = Class.forName(className);
			if (edu.umn.genomics.table.TableModelView.class
				.isAssignableFrom(theClass)) {
			    addDefaultViewClass(viewName, Class
				    .forName(className));
			}
			String icon = properties.getProperty(id + ".icon24");
			if (icon != null) {
			    defaultViewIcons24.put(theClass, new ImageIcon(cl
				    .getResource(icon)));
			}
			icon = properties.getProperty(id + ".icon16");
			if (icon != null) {
			    defaultViewIcons16.put(theClass, new ImageIcon(cl
				    .getResource(icon)));
			}
		    } catch (Exception ex) {
			System.err.println("setDefaultViews " + viewName + " "
				+ className + "\n " + ex);
     ...
     * 
     * @param viewName
     *            The name this view is referred as.
     * @param viewClass
     *            The viewing class.
     */
     ...
     *            The name this view is referred as.
     * @param className
     *            The fully qualified class name for the viewing class.
     */
	...
	if (edu.umn.genomics.table.TableModelView.class
	    ...
	    throw new ClassCastException(viewClass.toString()
		    + " doesn't implement "
     ...
     * 
     * @param viewName
     *            The name for the view to be removed.
     ...
     *            The viewing class.
     * @param icon
     *            The icon for this viewing class.
     */
	...
	if (obj instanceof JFrame) {
	    try {
		return getViewIcon((((JFrame) obj).getContentPane()
			.getComponent(0).getClass()), viewIcons16);
	    } catch (Exception ex) {
	    }
	} else if (obj instanceof TableModel) {
	    TableModel tm = (TableModel) obj;
	    for (; tm instanceof VirtualTableModel; tm = ((VirtualTableModel) tm)
		    .getTableModel())
		;
	...
	if (treeNode != null) {
	    ...
	    if (obj instanceof JFrame) {
		try {
		    Component c = ((JFrame) obj).getContentPane().getComponent(
			    0);
     ...
     * 
     * @param obj
     *            The object for which to get a tree node.
     ...
     * 
     * @param parent
     *            The parent object, if null add the child to the root node.
     * @param child
     *            The object to add as a child node in the tree.
     * @return The tree node of the child object.
     */
    private DefaultMutableTreeNode addNode(Object parent, Object child) {
	DefaultMutableTreeNode pn = parent == null ? (DefaultMutableTreeNode) dtm
		.getRoot()
		: getNode(parent);
     ...
     * 
     * @param tn
     *            The tree node to remove and destroy.
     */
	...
	if (tn != null) {
		...
		if (obj instanceof JFrame) {
		    ...
		    tm_vtm.remove(tm);
		    if (vtm.getTableModel() != tm
			    && tm_vtm.containsKey(vtm.getTableModel())) {
     ...
     * 
     * @param tm
     *            the TableModel for which to return a VirtualTableModelProxy.
	...
	if (useSelection) {
	    count = 0;
	    for (int i = rows.getMinSelectionIndex(); i <= rows
	    ...
	    vtm.setName("Selected " + count + " Rows of "
		    + getTableModelProxy(tm).getName());
	    vtm.setIndexMap(map);
	...
	try {
		...
		if (jc instanceof TableModelView) {
		    ...
		    ((TableModelView) jc)
			    .setSelectionModel(getRowSelectionModel(vtm));
     ...
     * @param view
     *            The compenent to be placed in the frame to be viewed.
	...
	int pw = Math.min(sw, Math.max(d.width, sw / 10 * 4)) - insets.left
		- insets.right;
	int ph = Math.min(sh, Math.max(d.height, sh / 10 * 4)) - insets.top
		- insets.bottom;
	setFrameBounds(frame, ((sw - pw) / 2), ((sh - ph) / 2), pw, ph);
     ...
     * 
     * @param frame
     *            The frame to which the toolbar is to be added.
     * @param view
     *            The view to be saved as an image.
     */
     ...
     *            The view to be saved as an image.
     * @param extraButtons
     *            Extra buttons to be added to the toolbar.
     */
    public static void setViewToolBar(Window frame, Component view,
	...
	jBtn.addActionListener(new ActionListener() {
		...
		WindowEvent we = new WindowEvent(theframe,
	...
	if (view != null) {
		...
		jBtn.addActionListener(new ActionListener() {
			...
			try {
			    SaveImage
				    .saveImage(comp instanceof TableModelView ? ((TableModelView) comp)
					    .getCanvas()
					    : comp);
			} catch (IOException ioex) {
			    JOptionPane.showMessageDialog(comp,
				    ioex.toString(), "Save Image",
		...
		try {
			...
			public void actionPerformed(ActionEvent e) {
				...
				args[0] = comp instanceof TableModelView ? ((TableModelView) comp)
					.getCanvas()
					: comp;
				Class.forName(
					"edu.umn.genomics.component.SavePDF")
					.getMethod("savePDF", paramClass)
					.invoke(null, args);
			    } catch (Exception ex) {
				JOptionPane.showMessageDialog(comp, comp
					.getClass()
		    ...
		    jBtn
			    .setToolTipText("Save PDF requires the Lowagie iText package");
		    jBtn.setEnabled(false);
		}
		jtb.add(jBtn);
	    }
     ...
     *            The TableModel for which to get the selection model.
     * @param ht
     *            The Hashtable in which to store the ListSelectionModel.
     * @return The selection model for this TableModel.
     */
	...
	return tm == null ? false : tm_vtm.containsKey(tm)
		|| tm_vtm.contains(tm);
    }
...
class DbgDefaultListSelectionModel extends DefaultListSelectionModel {
	...
	System.err.println(" +++ " + l + " >>> "
		+ getListSelectionListeners().length + " " + this.hashCode());
