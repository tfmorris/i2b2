...
public class TimeConstrainFrame extends javax.swing.JFrame {
	...
	public TimeConstrainFrame(QueryConceptData data_, KTable table_) {
		data = data_;
		table = table_;
	...
	private void jOKButtonActionPerformed(java.awt.event.ActionEvent evt) {
	    if (jToDateCheckBox.isSelected() && jFromDateCheckBox.isSelected()
		    && jToDateField.getDateInMillis()<jFromDateField.getDateInMillis()) {
		
		JOptionPane.showMessageDialog(this, "The to time is before the from time, please change that.", 
		...
		if (this.jToDateCheckBox.isSelected()) {
			...
			data.endMonth(toCalendar.get(Calendar.MONTH) + 1);
		...
		if (this.jFromDateCheckBox.isSelected()) {
			...
			data.startMonth(fromCalendar.get(Calendar.MONTH) + 1);
		...
		String timeStr = " -> ";
		if (data.startTime() != -1) {
			String tmp = data.startMonth() + "/" + data.startDay() + "/"
					+ data.startYear() + timeStr;
			timeStr = tmp;
		}
		if (data.endTime() != -1) {
			timeStr += data.endMonth() + "/" + data.endDay() + "/"
					+ data.endYear();
		}
