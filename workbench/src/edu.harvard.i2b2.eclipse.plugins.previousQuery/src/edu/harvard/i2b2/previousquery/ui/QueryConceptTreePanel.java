...
import edu.harvard.i2b2.previousquery.data.QueryConceptTreeData;
								   				...
								   				ActionListener
{
              ...
              try {
            	  SAXBuilder parser = new SAXBuilder();
	  		      ...
	  		      for (Iterator itr=conceptChildren.iterator(); itr.hasNext(); ) {
	  		    	  ...
	  		    	  Element lookupdbXml = (Element) conceptXml.getChild("lookupdb");
	  		    	  String lookupdb = lookupdbXml.getText().trim();
	  		    	  Element lookuptableXml = (Element) conceptXml.getChild("lookuptable");
	  		    	  String lookuptable = lookuptableXml.getText().trim();
	  		    	  Element selectserviceXml = (Element) conceptXml.getChild("selectservice");
	  		    	  String selectservice = selectserviceXml.getText().trim();
	  					
	  		    	  Element conTableXml = (Element) conceptXml.getChildren().get(0);
	  		    	  Element visualAttribs = conTableXml.getChild("c_visualattributes");
	...
	private void populateChildNodes(DefaultMutableTreeNode node) {
		try {
			org.w3c.dom.Document queryResultDoc = null;//QuerySelectServiceClient.getXMLResult(data, 0);		
