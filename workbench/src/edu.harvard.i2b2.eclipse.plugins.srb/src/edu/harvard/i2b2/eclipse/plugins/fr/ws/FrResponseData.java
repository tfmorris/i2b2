...
import edu.harvard.i2b2.fr.datavo.fr.query.SendfileResponseType;


abstract public class FrResponseData {

	public static final String THIS_CLASS_NAME = FrResponseData.class.getName();
    private Log log = LogFactory.getLog(THIS_CLASS_NAME);	
    private ResponseMessageType respMessageType = null;
    
	public FrResponseData() {}
	...
	public SendfileResponseType doReadSendfile(){
		SendfileResponseType loads = null;
		try {
			BodyType bodyType = respMessageType.getMessageBody();
			JAXBUnWrapHelper helper = new JAXBUnWrapHelper(); 
			if(bodyType != null)
		        loads = (SendfileResponseType) helper.getObjectByClass(bodyType.getAny(),
	...
	public String getXMLString(LoadType load)throws Exception{ 
		...
		try {
			FRJAXBUtil.getJAXBUtil().marshaller(load, strWriter);
		} catch (JAXBUtilException e) {
