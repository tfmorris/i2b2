...
import edu.harvard.i2b2.eclipse.plugins.exportData.ontologyMessaging.GetCodeInfoResponseMessage;
...
public class ObservationKTableModel implements KTableModel {
	...
	content.put("0/0", "Index");
	content.put("1/0", "Name");
	content.put("2/0", "Time");
	...
	/*
	 * if ((col == 5)&&(row>0)) { KTableCellEditorColor e = new
	 * KTableCellEditorColor(); return e; } else if ((col == 4)&&(row>0)) {
	 * KTableCellEditorCombo combo = new KTableCellEditorCombo();
	 * combo.setItems(new String[]{"Very Tall", "Tall", "Medium", "Low",
	 * "Very Low"}); return combo; } else if ((col == 2)&&(row>0)) {
	 * KTableCellEditorCombo combo = new KTableCellEditorCombo();
	 * combo.setItems(new String[]{"NVAL_NUM", "TVAL_CHAR", "Location_CD",
	 * "Confidence_NUM", "Location_Path", "InOut_CD"}); return combo; } else
	 * if ((col == 3)&&(row>0)) { KTableCellEditorComboW combo = new
	 * KTableCellEditorComboW(); combo.setItems(new String[]{" = ", " > ",
	 * " < ", " between and ", " like "}); return combo; } else {
	 */
    ...
    // populate table from pdo set
    public void populateTable(ArrayList<ObservationType> oset) {
	deleteAllRows();
	...
	for (int i = 0; i < oset.size(); i++) {
	    ...
	    ObservationType obs = oset.get(i);

	    // check valtype
	    setContentAt(0, newRow + 1, new Integer(newRow + 1).toString());
	    setContentAt(1, newRow + 1, obs.getConceptCd().getName());
	    // getCodeInfo(obs.getConceptCd().getValue()));
	    // newRow+1,
	    // obs.getTvalChar().substring(obs.getTvalChar().lastIndexOf
	    // ("/")+1));
	    setContentAt(2, newRow + 1, obs.getStartDate().toString());
	    newRow++;
	    // }
	}
    }
	...
	for (int i = 1; i < rowCount; i++) {
	    ...
	    row.rowNumber = new Integer((String) content.get("0/" + i))
		    .intValue();
	    row.patientSetNumber = (String) content.get("1/" + i);
	    row.rowId = i;
