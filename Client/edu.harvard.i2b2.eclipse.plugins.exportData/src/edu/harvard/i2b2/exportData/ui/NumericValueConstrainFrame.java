...
public class NumericValueConstrainFrame extends javax.swing.JFrame {
	private QueryConceptData data;
	private KTable table;

	/** Creates new form ValueConstrainFrame */
	public NumericValueConstrainFrame(QueryConceptData data_,
			KTable table_) {// QueryConceptTreePanel parent) {
		data = data_;
		table = table_;
		...
		if (!data.valuePropertyData().okToUseValueFlag()) {
			jFlagRadioButton.setEnabled(false);
		}

		if (!data.valuePropertyData().okToUseValue()) {
			jNumericRadioButton.setEnabled(false);
		}

		jUnitsComboBox.removeAllItems();
		for (int i = 0; i < data.valuePropertyData().units.size(); i++) {
			jUnitsComboBox.addItem(data.valuePropertyData().units.get(i));
		}
	...
	{
		if (data.valuePropertyData().useNumericValue())
		{
			jOperatorComboBox.setSelectedItem(data.valuePropertyData().operator());

			jNumericRadioButton.setSelected(true);
			jNumericRadioButtonActionPerformed(null);
			jOperatorComboBoxActionPerformed(null);
			if ( data.valuePropertyData().operator().equals("BETWEEN"))
			{
				jLowValueTextField.setText(data.valuePropertyData().lowValue());
				jHighTextField.setText(data.valuePropertyData().highValue());
			} else //if ( data.valuePropertyData().value().equals("L"))
			{
				jValueTextField.setText(data.valuePropertyData().value());
			}
		} else 		if (data.valuePropertyData().useValueFlag())
		{
			if ( data.valuePropertyData().value().equals("H"))
			{
		...
		if (data.hasValue() && data.valuePropertyData().unit() != null)
		{
			jUnitsComboBox.setSelectedItem(data.valuePropertyData().unit());
		}
	...
	private void jOKButtonActionPerformed(java.awt.event.ActionEvent evt) {
		// QueryConceptTreeNodeData data = null;//data;
		...
		if (jNoValueRadioButton.isSelected()) {
			...
			if (operator.equalsIgnoreCase("between")) {
				...
				data.valueName("between " + data.valuePropertyData().lowValue()
						+ " and " + data.valuePropertyData().highValue());
			} else {
