...
public class WaitPanel extends javax.swing.JPanel implements Runnable {
    ...
    public void init(int width_, int height_) {
	int width;
	int height;
	...
	width = (int) (getParent().getWidth() * 0.40);
	height = (int) (getParent().getHeight() * 0.40);
	...
	add(topLabel1);
	topLabel1.setBounds(width - 100, height - 50, 400, 40);
	topLabel1.setText("Click on \"select patients\" tab to abort");
