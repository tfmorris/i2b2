...
public class EnumValueConstrainFrame extends javax.swing.JFrame {
	...
	public EnumValueConstrainFrame(QueryConceptData data_, KTable table_) {
		data = data_;
		table = table_;
		...
		// QueryConceptTreeNodeData data = new
		// QueryConceptTreeNodeData();//null;//= parent_.currentData();
	...
	{
		if (data.valuePropertyData().hasEnumValue())
		{
			if (data.valuePropertyData().useValueFlag())
			{
				jFlagRadioButton.setSelected(true);
			} else if (data.valuePropertyData().useTextValue())
			{
				...
				for (int i = 0; i < jEnumValueTable.getRowCount(); i++) {
					for (String selectedValue :data.valuePropertyData().selectedValues)
					{
