 ...
 * 
 * 
 * @see edu.harvard.i2b2.exportData.dataModel.KTableModel
 * @see edu.harvard.i2b2.explorer.dataModel.exportData.dataModel.KTableCellRenderer
 * @see edu.harvard.i2b2.exportData.dataModel.KTableCellEditor
 * @see edu.harvard.i2b2.exportData.dataModel.KTableCellSelectionListener The
 *      idea of KTable is to have a flexible grid of cells to display data in
 *      it. The class focuses on displaying data and not on collecting the data
 *      to display. The latter is done by the KTableModel which has to be
...
public class KTable extends Canvas {
    ...
    public void clearSelection() {/*
				   * if (m_MultiSelectMode) { if
				   * (m_Selection.size() < m_RowsFullyVisible
				   * m_ColumnsVisible) { if (m_RowSelectionMode)
				   * { for (Iterator iter =
				   * m_Selection.values().iterator();
				   * iter.hasNext();) { int row = ((Integer)
				   * iter.next()).intValue(); if (row >=
				   * m_TopRow && row <
				   * m_TopRow+m_RowsFullyVisible) { } } } else {
				   * for (Iterator iter =
				   * m_Selection.values().iterator();
				   * iter.hasNext();) { Point element = (Point)
				   * iter.next(); } } } }
				   */
	clearSelectionWithoutRedraw();
	m_FocusCol = -1;
     ...
     * 
     * @see edu.harvard.i2b2.exportData.dataModel.KTableModel for more
     *      information.
     * @param model
     */
