 ...
 * columns of labels.
 *<p>
 * The only visible method is <code>layout</code> that compute the
 * optimal positions of labels on the list.
 * <p>
 * The algorithm works that way:<p>
 * <ul>
 * <li> The labels are initially placed on the circle at the position
 * closest to the item they point to.
 * <li> We move them closer the focus radius first.
 * <li> We order them from left to right in trigonometric order,
 * i.e. we associate a real value to each item where
 * labels on the left have negative
 * values and the ones on the right have positive values.  The values
 * increase when going downwards on the left and upwards on the right,
 * being 0 at the bottom.
 * <li> We sort the vector according to this order and place  left
 * labels on the left and right labels on the right.
 * <li> Labels are left aligned, so for the left label, we have a bit
 * more work than for the right labels.
 * <li> For the left labels, we compute their max widths to have the
 * left position where all the labels are positioned.
 * <li> We then add two new lines to each
 * <code>LiteDisplacedLabel</code>: one that goes horizontally from the
 * right of the label to the right of the longest label and the second
 * which connects this one to the endpoint of the original line (on
 * the focus circle).
 * <li> For the right labels, all the labels are left aligned and only
 * only line is added to each <code>LiteDisplaedLabel</code>.
 * <li> Each label is not only left aligned, but also stacked on top
 * of each others.  Once this is done, we compute the best vertical
 * origin by finding the smallest distance of all the labels to their
 ...
 */
public
class RectLayout extends LabelLayout {
  boolean stable_x;
  int column;
  int [] offsets;
...
public int do_layout() {
  ...
  protected int compute_left_right(double order[]) {
    ...
    for (left_right = 0; left_right < nlabel && order[left_right] < 0;
	 left_right++) 
      /* do nothing */ ;
  ...
  /**
   * Move each <code>LiteDisplacedLabel</code> to another circle
   * radius.
   *
   * @param	v	List of <code>LiteDisplacedLabel</code>.
  ...
  /**
   * Align left of the <code>LiteDisplacedLabel</code> objects from
   * index 0 to index <code>last</code>.
   */
  public void align_left(double order[], int last, int max_width) {
    ...
    int i;
    int height = ((LiteDisplacedLabel)v.firstElement()).
      getLabel().getBounds().height;

    optimize_origin(order, height, first, last);

    int offset = compute_offset(order[first] + center.y + radius - height/2,
				order[last-1] + center.y + radius + height/2);
    ...
    for (i = first; i < last; i++) {
      ...
      if (l.getItem() == 2) {
          ...
          l.addElement(new LiteLine(p, p2,  
				ll.getThickness(), ll.getColor()));
          l.addElement(new LiteLine(p2, ll.getPosition(),
				ll.getThickness(), ll.getColor()));
  ...
  public void align_right(double order[], int first) {
    ...
    for (i = first; i < last; i++) {
      ...
      if (l.getItem() == 2)  {
          LiteLine ll = l.getLine();
          l.addElement(new LiteLine(p, ll.getPosition(),
				ll.getThickness(), ll.getColor()));
          //l.getLine().setPosition(p);
      }
    }
  ...
  public void setStableX(boolean sx) { stable_x = sx; }
  public boolean getStableX() { return stable_x; }
