...
import edu.harvard.i2b2.analysis.ui.TimelineSearchFrame;
...
// public class record extends Applet implements newApplet,Runnable{ 
    ...
    // booleans for facet and facetLine read in as parameters in init.. added 1/6/98

    static boolean[] option = new boolean[5]; 
    static boolean[] lbloption = new boolean[3];   
    static boolean excentric;   // is not included in lbloption because it is dynamic
    static boolean[] column = new boolean[2];
    static boolean angle_label, infotip;
    ...
    // these booleans are read from the html file in init() (see that function for
    // parameter names
    ...
    public void run() {
       ...
       while(Thread.currentThread() == theThread) {
           if(changed && threadTest) {
                theTabPanel.theTimeLinePanel.repaint();
                changed = false;
           }
           try {
	            Thread.sleep(1);
    ...
    public void init(String recordData) {
        ...
        if (recordData==null)
        {
	        ...
	        if(datafile == null)
	           //datafile = "http://www.cs.umd.edu/projects/hcil/Research throw /1997/dandemo/width.old";
        ...
        // does this one make a difference? All the others panels are in toppanel
        ...
        // no layout is set, so reshapes are not needed to get anything to display
        // if a layout is set... use preferredsize or some other solution (like a good layout manager?) 
        ...
        // tabpanel actually goes to www for data within timelinepanel, actually this
        // method sets the applet for the timelinepanel (so can use showstatus, showdocument,
        // newapplet methods... actually applet methods, but are using inside a non applet
        // derived class
   ...
   //* inherited from newApplet interface
    ...
    public void showDocument(String theURL) {
        try {
        	...
        	System.out.println("Exception while opening URL "+theURL +" : "+e);
        }
    }
    ...
    public void setWH(int width,int height) {
      ...
      // I needed this function because compiler complained that I couldn't call
      // resize from a static method (which it declared the one in newApplet to be)
      //setSize(resizeWidth, resizeHeight);
    }
    
    private String getParameter(String requestedString) {
    	if (requestedString.equalsIgnoreCase("bgcolor")) return "ffffff";
    	if (requestedString.equalsIgnoreCase("bgcolor")) return "ffffff";
    	if (requestedString.equalsIgnoreCase("bgcolor")) return "ffffff";
    	if (requestedString.equalsIgnoreCase("fgcolor")) return "000000";
    	if (requestedString.equalsIgnoreCase("hlcolor")) return "99ccff";
    	if (requestedString.equalsIgnoreCase("textcolor")) return "000000";
    	if (requestedString.equalsIgnoreCase("datafile")) return "i2b2xml.lld";
    	if (requestedString.equalsIgnoreCase("quick_compact")) return "false"; // ?
    	if (requestedString.equalsIgnoreCase("slow_compact")) return "false"; // vertically optimizes layout for compactness
    	if (requestedString.equalsIgnoreCase("time_ordered")) return "false"; // eveything has its own line and arranged cascading down
    	if (requestedString.equalsIgnoreCase("agg_ordered")) return "true"; // every different concept has it's own line
    	if (requestedString.equalsIgnoreCase("comments")) return "false";
    	if (requestedString.equalsIgnoreCase("top_right")) return "true";
    	if (requestedString.equalsIgnoreCase("top_right_left")) return "false"; // labels start on left?
    	if (requestedString.equalsIgnoreCase("4corners")) return "false";
    	if (requestedString.equalsIgnoreCase("no_arrow")) return "true";
    	if (requestedString.equalsIgnoreCase("arrow")) return "false"; // line connects label to bar
    	if (requestedString.equalsIgnoreCase("delay_time")) return "true";
    	if (requestedString.equalsIgnoreCase("real_time")) return "false";
    	if (requestedString.equalsIgnoreCase("variable_length")) return "true";
    	if (requestedString.equalsIgnoreCase("fix_length")) return "false";
    	if (requestedString.equalsIgnoreCase("summarylabel")) return "true"; // bold labels
    	if (requestedString.equalsIgnoreCase("summarytimeline")) return "true";
    	if (requestedString.equalsIgnoreCase("label_only_result")) return "false";
    	if (requestedString.equalsIgnoreCase("timeline_gray")) return "true";
    	if (requestedString.equalsIgnoreCase("timeline_remove")) return "false";
    	if (requestedString.equalsIgnoreCase("label_gray")) return "true";
    	if (requestedString.equalsIgnoreCase("label_remove")) return "false";
    	if (requestedString.equalsIgnoreCase("symbol")) return "false";
    	if (requestedString.equalsIgnoreCase("vague_association")) return "false";
    	if (requestedString.equalsIgnoreCase("angle_label")) return "false"; // makes blank?
    	if (requestedString.equalsIgnoreCase("infotip")) return "true"; // Tooltip hovering over bar
    	if (requestedString.equalsIgnoreCase("excentric")) return "false"; // very cool label attached under box
    	if (requestedString.equalsIgnoreCase("width")) return "700";
    	if (requestedString.equalsIgnoreCase("height")) return "685";
