...
public class scale{
    ...
    public void setMode(MyDate dateMin, MyDate dateMax){
        ...
        mode = "decade";
        if(d <= 60 * 24 * 365 * 10) mode = "year";
        if(d <= 60 * 24 * 365) mode = "month";
        if(d <= 60 * 24 * 14)
                mode = "day";
        if(d <= 60 * 12) mode = "hour";
       // if(d <= 20) mode = "min";
        if(d <= 60 * 12) mode = "disabled";
    }
        ...
        if(mode.equals("decade")){
            for(i=0; i <= MAX_TICKS; i++){
                if(i == 0) theTicks[i] = 0;
                    ...
                    currDate.setYear(DateMin.getYear()-DateMin.getYear()%10+10);
                    currDate.setMonth(1);
                    currDate.setDay(1);
                    currDate.setHour(0);
                    currDate.setMin(0);
                    theTicks[i] = (int)Math.round((double)DateMin.MinDiff(currDate)*width/
                                               DateMin.MinDiff(DateMax));  // positions for vertical grid bars (background)
                }
                else{
                ...
                if(currDate.getMonth() != 1)
                     ...
                     +((currDate.getYear()-1900>=100)?(currDate.getYear()-2000):(currDate.getYear()-1900));
                else theLabelString[i] = "" + ((currDate.getYear()-1900>=100)?(currDate.getYear()-2000):(currDate.getYear()-1900));
            }
        }

        if(mode.equals("year")){
            if (DateMin.getYear()==1996/* && DateMin.getMonth()==1 && DateMin.getDay()==1*/)
                  i=0;
            for(i=0; i <= MAX_TICKS; i++){
                if(i ==0) theTicks[i] = 0;
                else if(i == 1){
            ...
            for(i=0; i <= MAX_TICKS; i++){
                if(i ==0) theTicks[i] = 0;
                    ...
                    currDate.setYear(currDate.getYear()+currDate.getMonth()/12);
            ...
            for(i=0; i <= MAX_TICKS; i++){
                ...
                theLabelString[i] = "" + currDate.getMonth() + "/" + currDate.getDay();
            }
        }
            ...
            for(i=0; i <= MAX_TICKS; i++){
                ...
                theLabelString[i] = "" + currDate.getHour() + ":" + currDate.getMin();
            }
        }
    }
/*
    public void setLowerScale(MyDate DateMin, MyDate DateMax){
        int start_decade, start_year, start_month, start_day;
        int i;

        setMode(DateMin, DateMax);
        theTicks = new int[n_ticks + 1];
        theLabelString = new String[n_ticks + 1];

        for(i=0; i <= n_ticks; i++)
           theTicks[i] = (int)Math.round((double)(i * (width-1))/ n_ticks);

        if(mode.equals("decade")){
            start_decade = DateMin.getYear() - DateMin.getYear()%10;
            for(i=0; i <= n_ticks; i++)
                theLabelString[i] = "" + (start_decade + i*10);
        }

        if(mode.equals("year")){
            start_year = DateMin.getYear();
            for(i=0; i <= n_ticks; i++)
                theLabelString[i] = "" + (start_year +i - 1900);
        }

        else if(mode.equals("month")){
            start_year = DateMin.getYear();
            start_month = DateMin.getMonth();
            for(i=0; i <= n_ticks; i++)
                theLabelString[i] = "" + ((start_month +i-1)%12+1) + "/" +
                (start_year - 1900 +(start_month+i-1)/12);
        }

        else if(mode.equals("day")){
            start_year = DateMin.getYear();
            start_month = DateMin.getMonth();
            start_day = DateMin.getDay();
            for(i=0; i <= n_ticks; i++)
                theLabelString[i] = "" +
                (start_month + (start_day +i -1)/31) + "/" +
                ((start_day + i - 1)%31 + 1);
        }
    }
*/
