...
public class ModifierFolderWizard extends Wizard {
	private static final Log log = LogFactory.getLog(ModifierFolderWizard.class);
	private boolean okToFinish = false;

	public ModifierFolderWizard() {
		setWindowTitle(Messages.getString("ModifierFolderWizard.WindowTitle")); //$NON-NLS-1$
	...
	public void addPages() {
		...
		addPage(new ModifierFolderSettingsPage());
	...
	public IWizardPage getNextPage(IWizardPage page) {
		if (page instanceof ModifierFolderSettingsPage) {
			((ModifierFolderSettingsPage) page).updateMetadataRecord();
			okToFinish = false;
		}
	...
	public boolean performFinish() {
		if (!(Roles.getInstance().isRoleValid())){
			...
			mBox.setText("New Modifier Folder Warning");
...
//						ModifierComposite.getInstance().refresh();
	...
	public void add(final Display theDisplay)
	{
		try {
			...
			while(procStatus == null || !procStatus.getType().equals("DONE")){
				...
				String response = OntServiceDriver.addModifier(MetadataRecord.getInstance().getMetadata().getModifier());
...
class ModifierFolderSettingsPage extends WizardPage {
	...
	public static final String PAGE_NAME = "ModifierFolderSettings"; //$NON-NLS-1$

	public ModifierFolderSettingsPage() {
		super(PAGE_NAME); 

		//		String notice = "Selected Parent is: " + MetadataRecord.getInstance().getMetadata().getKey();			
		setTitle(Messages.getString("ModifierFolderWizard.ItemSettings"));// + "\n\n" + notice);
...
//		new Label(parentInfo, SWT.NONE).setImage(image);
		...
		new Label (itemSettings, SWT.NONE).setText("*Modifier Folder Name:");			
		text1 = new Text(itemSettings, SWT.BORDER);
		text1.setToolTipText("The Modifier Folder Name is the name that appears in the navigate tree.");
	...
	public void performHelp(){
		...
		String EDIT_VIEW_CONTEXT_ID = PREFIX + ".edit_terms_view_help_wizard_folder";
...
//		MetadataRecord.getInstance().getMetadata().setValuetypeCd(getValuetypeCd());
		...
		if((MetadataRecord.getInstance().getParentData().getModifier() == null) ||
				((MetadataRecord.getInstance().getParentData().getModifier().getName() == null))){
			appliedPath = StringUtil.getPath(MetadataRecord.getInstance().getParentData().getFullName());
