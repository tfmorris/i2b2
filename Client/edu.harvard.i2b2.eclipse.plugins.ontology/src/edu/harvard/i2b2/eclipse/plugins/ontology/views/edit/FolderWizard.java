...
public class FolderWizard extends Wizard {
	private static final Log log = LogFactory.getLog(FolderWizard.class);
	private boolean okToFinish = false;

	public FolderWizard() {
		setWindowTitle(Messages.getString("FolderWizard.WindowTitle")); //$NON-NLS-1$
	...
	public void addPages() {
		...
		addPage(new FolderSettingsPage());
...
//		addPage(new ValueMetadataSettingsPageOne());
	...
	public IWizardPage getNextPage(IWizardPage page) {
		if (page instanceof FolderSettingsPage) {
			((FolderSettingsPage) page).updateMetadataRecord();
			okToFinish = false;
		}
...
*/
	...
	public boolean performFinish() {
		if (!(Roles.getInstance().isRoleValid())){
			...
			mBox.setText("New Folder Warning");
...
//				TABLE_ACCESS_DENIED and USER_INVALID and DATABASE ERRORS
	...
	class FolderSettingsPage extends WizardPage {
		...
		public static final String PAGE_NAME = "FolderSettings"; //$NON-NLS-1$
		
		public FolderSettingsPage() {
			super(PAGE_NAME); 
			
	//		String notice = "Selected Parent is: " + MetadataRecord.getInstance().getMetadata().getKey();			
			setTitle(Messages.getString("FolderWizard.ItemSettings"));// + "\n\n" + notice);
