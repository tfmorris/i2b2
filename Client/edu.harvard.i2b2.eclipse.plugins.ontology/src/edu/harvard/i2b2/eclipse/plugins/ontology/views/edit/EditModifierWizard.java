...
public class EditModifierWizard extends Wizard {
	private static final Log log = LogFactory.getLog(EditModifierWizard.class);
	private boolean okToFinish = false;

	public EditModifierWizard() {
		setWindowTitle(Messages.getString("EditModifierWizard.WindowTitle")); //$NON-NLS-1$
	...
	public void addPages() {
		...
		addPage(new EditModifierSettingsPage());
	...
	public IWizardPage getNextPage(IWizardPage page) {
		if (page instanceof EditModifierSettingsPage) {
			((EditModifierSettingsPage) page).updateMetadataRecord();
			okToFinish = false;
		}
...
//				TABLE_ACCESS_DENIED and USER_INVALID and DATABASE ERRORS
	...
	class EditModifierSettingsPage extends WizardPage {
		...
		public static final String PAGE_NAME = "EditModifierSettings"; //$NON-NLS-1$
		
		public EditModifierSettingsPage() {
			super(PAGE_NAME); 
			
	//		String notice = "Selected Parent is: " + MetadataRecord.getInstance().getMetadata().getKey();			
			setTitle(Messages.getString("EditModifierWizard.ItemSettings"));// + "\n\n" + notice);
		...
		public void createControl(Composite parent) {
			...
			new Label(itemSettings, SWT.NONE).setText(StringUtil.getPath(MetadataRecord.getInstance().getParentData().getModifier().getKey()));
			...
			text1.setText(MetadataRecord.getInstance().getParentData().getModifier().getName());
			...
			text1.addModifyListener(new ModifyListener() {
			      ...
			      public void modifyText(ModifyEvent event) {
			    	  if(typeCombo.getText().equals("MODIFIER")){
	...
	/*	    final Combo schemesCombo = new Combo(conceptCodeComp,SWT.READ_ONLY);
		...
		*/	
			...
			if(MetadataRecord.getInstance().getParentData().getModifier().getBasecode() != null){
				text2.setText(MetadataRecord.getInstance().getParentData().getModifier().getBasecode());
			}
	...
	/*		schemesCombo.addSelectionListener(new SelectionListener(){
			...
			*/
			...
			String tooltip = MetadataRecord.getInstance().getParentData().getModifier().getTooltip();
			...
			String visAttribute = MetadataRecord.getInstance().getParentData().getModifier().getVisualattributes();
			if(visAttribute.startsWith("D"))
				typeCombo.setText("FOLDER");
			else if(visAttribute.startsWith("O"))
				typeCombo.setText("CONTAINER");
			else if(visAttribute.startsWith("R"))
				typeCombo.setText("MODIFIER");
	...
	//		MetadataRecord.getInstance().getMetadata().setValuetypeCd(getValuetypeCd());
			...
			MetadataRecord.getInstance().getMetadata().getModifier().setVisualattributes(getVisualAttribute());
			MetadataRecord.getInstance().getMetadata().getModifier().setLevel(MetadataRecord.getInstance().getParentData().getModifier().getLevel());
			MetadataRecord.getInstance().getMetadata().getModifier().setBasecode(getBasecode());
