...
public class ModifierItemWizard extends Wizard {
	private static final Log log = LogFactory.getLog(ModifierItemWizard.class);
	private boolean okToFinish = false;

	public ModifierItemWizard() {
		// this enables a square, plain help button
		//	setHelpAvailable(true);
		
		setWindowTitle(Messages.getString("ModifierItemWizard.WindowTitle")); //$NON-NLS-1$
	...
	public void addPages() {
		...
		addPage(new ModifierItemSettingsPage());
		addPage(new SynonymsPage());
		addPage(new QueryDimensionSettingsPage());
	//	addPage(new ValueMetadataSettingsPageOne());  // keyword entry no longer used
		addPage(new ValueMetadataSettingsPageTwo());    //datatype
		addPage(new ValueMetadataSettingsPageThree());  // enum
		addPage(new ValueMetadataSettingsPageFour());   // ranges
		addPage(new ValueMetadataSettingsPageFive());   // units
		addPage(new VerifyModifierDataPage());
	...
	public IWizardPage getNextPage(IWizardPage page) {
		...
		if (page instanceof ModifierItemSettingsPage) {
			((ModifierItemSettingsPage) page).updateMetadataRecord();
			okToFinish = false;
		}
			...
			if (!(MetadataRecord.getInstance().isValueMetadataFlag())){
				...
				VerifyModifierDataPage verifyPage = (VerifyModifierDataPage) getPage(VerifyModifierDataPage.PAGE_NAME);
...
/*		else if (page instanceof ValueMetadataSettingsPageOne) {
	...
	*/	
	...
	public Thread addChild(){
		...
		return new Thread() {
			...
			public void run(){
				try {
					// update entry in UI
			//		ModifierComposite.getInstance().addChildModifier(MetadataRecord.getInstance().getMetadata().getModifier());
					// update entry in db
					ModifierType node = MetadataRecord.getInstance().getMetadata().getModifier();
				//	node.setSynonymCd("N");
					add(theDisplay, node);
				...
				theDisplay.syncExec(new Runnable() {
					public void run() {
						if (MetadataRecord.getInstance().getParentData().getModifier() == null)
							ModifierComposite.getInstance().addModifiers(MetadataRecord.getInstance().getParentData());
						else
							ModifierComposite.getInstance().addChildModifier(MetadataRecord.getInstance().getMetadata().getModifier());
	...
	public boolean validate()
	{
		...
		try {	
			...
			while(procStatus == null || !procStatus.getType().equals("DONE")){
				...
				valid = modifiers.getModifier().isEmpty();						
				if(!valid){
					MessageBox mBox = new MessageBox(Display.getCurrent().getActiveShell(), SWT.ICON_INFORMATION | SWT.OK);
					mBox.setText("Please Note ...");
					mBox.setMessage("The path name you created for this modifier is not unique \n" + 
	...
	public void add(final Display theDisplay, ModifierType node)
	{
		try {
			OntologyResponseMessage msg = new OntologyResponseMessage();
			StatusType procStatus = null;	
				...
				while(procStatus == null || !procStatus.getType().equals("DONE")){
					...
					String response = OntServiceDriver.addModifier(node);
...
class ModifierItemSettingsPage extends WizardPage {
	...
	public static final String PAGE_NAME = "ModifierItemSettings"; //$NON-NLS-1$
	
	public ModifierItemSettingsPage() {
		super(PAGE_NAME); 
		setTitle(Messages.getString("ModifierItemWizard.ItemSettings"));// + "\n\n" + notice);
...
/*		Image image = null;
		...
		else if(MetadataRecord.getInstance().getParentData().getVisualattributes().startsWith("D")){
			ImageDescriptor imageDescriptorModifierFolder = ImageDescriptor.createFromFile(getClass(), "icons/modifierFolderPencil.jpg");
			image = imageDescriptorModifierFolder.createImage();	
		}
		else if(MetadataRecord.getInstance().getParentData().getVisualattributes().startsWith("O")){
			ImageDescriptor imageDescriptorModifierContainer = ImageDescriptor.createFromFile(getClass(), "icons/modifierContainterPencil.jpg");
			image = imageDescriptorModifierContainer.createImage();
		}
*/		new Label(itemSettings, SWT.NONE).setText("The concept path this modifier applies to will be: ");
	...
	//	new Label(parentInfo, SWT.NONE).setImage(image);
		if(MetadataRecord.getInstance().getParentData().getModifier() == null){
			new Label(parentInfo, SWT.NONE).setText(" " + StringUtil.getPath(MetadataRecord.getInstance().getParentData().getKey()));
				applyToAll = new Button (itemSettings, SWT.CHECK);
				applyToAll.setText("Apply to all descendents of this concept");
				GridData layoutData = new GridData();
				layoutData.horizontalSpan = 2;
				applyToAll.setLayoutData(layoutData);
		}

		else
			new Label(parentInfo, SWT.NONE).setText(" " + MetadataRecord.getInstance().getParentData().getModifier().getAppliedPath());

		new Label(itemSettings,SWT.NONE);
		new Label(itemSettings,SWT.NONE);
		
	//  PATH is relative to the modifier not the concept 

		new Label(itemSettings, SWT.NONE).setText("The modifier's path will be: ");
		if((MetadataRecord.getInstance().getParentData().getModifier() == null) ||
			((MetadataRecord.getInstance().getParentData().getModifier().getName() == null))){
			path = "\\";	
			new Label(itemSettings, SWT.NONE).setText(path);
			key = "\\\\" + StringUtil.getTableCd(MetadataRecord.getInstance().getParentData().getKey()) + path;
		}
		else{
			key = MetadataRecord.getInstance().getParentData().getModifier().getKey();
			new Label(itemSettings, SWT.NONE).setText(StringUtil.getPath(key));
		}
		new Label(itemSettings, SWT.NONE);
		new Label(itemSettings, SWT.NONE);

		
		new Label (itemSettings, SWT.NONE).setText("*Modifier Name:");			
		text1 = new Text(itemSettings, SWT.BORDER);
		text1.setToolTipText("The Modifier Name is the name that appears in the navigate tree.");
		...
		if((MetadataRecord.getInstance().getParentData().getModifier() != null) &&
				((MetadataRecord.getInstance().getParentData().getModifier().getName() != null)))
			tooltip = StringUtil.getTooltipBody(MetadataRecord.getInstance().getParentData().getModifier().getTooltip()) + " \\"; 

		text1.addModifyListener(new ModifyListener() {
		      public void modifyText(ModifyEvent event) {
		    	//  update tooltip and symbol name as name is entered
		    	  String name = text1.getText();
		    	  text5.setText(tooltip +  name);
		    	...
		    	// Page is not complete until a name, concept and symbol name have been added
		    	  if ( (text1.getText().length() > 0) && (text2.getText().length() > 0) && (text6.getText().length() > 0))
		...
		new Label (itemSettings, SWT.NONE).setText("*Modifier code:");
...
/*	    final Combo schemesCombo = new Combo(conceptCodeComp,SWT.READ_ONLY);
		...
		*/
		...
		text2.setToolTipText("Modifier codes identify concept modifiers within queries.");
...
/*		
	...
	public void performHelp(){
		...
		String EDIT_VIEW_CONTEXT_ID = PREFIX + ".edit_terms_view_help_wizard_item";
	...
	public void updateMetadataRecord(){
		MetadataRecord.getInstance().getMetadata().getModifier().setName(getName());
		MetadataRecord.getInstance().getMetadata().getModifier().setBasecode(getConceptCode());
		MetadataRecord.getInstance().getMetadata().getModifier().setSourcesystemCd(getSourcesystemCode());
		MetadataRecord.getInstance().getMetadata().getModifier().setComment(getComment());
		MetadataRecord.getInstance().setSymbol(getSymbol());
	//	MetadataRecord.getInstance().getMetadata().setValuetypeCd(getValuetypeCd());

		MetadataRecord.getInstance().getMetadata().getModifier().setKey(key + MetadataRecord.getInstance().getSymbol() + "\\");
		MetadataRecord.getInstance().getMetadata().getModifier().setDimcode(StringUtil.getPath(MetadataRecord.getInstance().getMetadata().getModifier().getKey()));

//		MetadataRecord.getInstance().setValueMetadataFlag(enterValues.getSelection());
		String appliedPath = "@";
		if((MetadataRecord.getInstance().getParentData().getModifier() == null) ||
				((MetadataRecord.getInstance().getParentData().getModifier().getName() == null))){
			appliedPath = StringUtil.getPath(MetadataRecord.getInstance().getParentData().getKey());
			if (applyToAll.getSelection())
				appliedPath += "%";
		}
		else{
			appliedPath = MetadataRecord.getInstance().getParentData().getModifier().getAppliedPath();
		}
		MetadataRecord.getInstance().getMetadata().getModifier().setAppliedPath(appliedPath);
		MetadataRecord.getInstance().getMetadata().getModifier().setTooltip(getTooltip());
	}
	...
	public String getTooltip() {
		...
		return text5.getText()+ " [" + MetadataRecord.getInstance().getMetadata().getModifier().getAppliedPath() + "]";
	}
