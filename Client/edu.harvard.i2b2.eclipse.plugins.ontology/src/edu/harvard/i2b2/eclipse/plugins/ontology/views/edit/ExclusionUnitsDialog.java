 ...
 * @author  lcp5
 */
public class ExclusionUnitsDialog extends Dialog {
    
	private Text nameBox;
    /**
     * Creates Keyword Dialog
     */
    public ExclusionUnitsDialog(Shell parentShell) {
    	super(parentShell);
	...
	protected Control createDialogArea(Composite parent){
    	...
    	comp.getShell().setText("Enter associated exclusion unit value");
    	
       	GridLayout layout = (GridLayout)comp.getLayout();
    	layout.numColumns = 2;
    	
    	Label synLabel = new Label(comp, SWT.RIGHT);
    	synLabel.setText("Exclusion Unit Value: ");
	...
	protected void buttonPressed(int buttonId){
    	...
    	// OK
    	if(buttonId == 0){
    		ValueMetadata.getInstance().setExcludingUnit(nameBox.getText());
