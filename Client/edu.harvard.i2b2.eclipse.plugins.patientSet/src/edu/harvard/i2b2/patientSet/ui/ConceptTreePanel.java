...
import edu.harvard.i2b2.patientSet.data.ConceptTreeData;
...
public class ConceptTreePanel extends javax.swing.JPanel implements
	...
	public ArrayList<QueryConceptTreeNodeModel> getItems() {
		/*
	...
	private DefaultMutableTreeNode addNode(QueryConceptTreeNodeModel node,
	...
	class MyRenderer extends DefaultTreeCellRenderer {
		...
		public Component getTreeCellRendererComponent(JTree tree, Object value,
			...
			if (node.getUserObject().getClass().getSimpleName()
				...
				QueryConceptTreeNodeModel nodeInfo = (QueryConceptTreeNodeModel) (node
						.getUserObject());
		...
		private ImageIcon getImageIcon(QueryConceptTreeNodeModel data) {
			String key = null;
			...
			try {
				...
				try {
					...
					for (Iterator itr = conceptChildren.iterator(); itr
						...
						QueryConceptTreeNodeModel node = new QueryConceptTreeNodeModel();
						node.name(c_name);
	...
	public void redraw(ConceptTreeData data) {
		...
		for (int i = 0; i < data.getItems().size(); i++) {
			QueryConceptTreeNodeModel node = data.getItems().get(i);
			addNode(node);
		}
	}
	...
	public void treeCollapsed(TreeExpansionEvent event) {
		...
		QueryConceptTreeNodeModel data = (QueryConceptTreeNodeModel) node
				.getUserObject();
	...
	public void treeExpanded(TreeExpansionEvent event) {
		...
		if (node.getChildCount() == 1) {
			final DefaultMutableTreeNode node1 = (DefaultMutableTreeNode) node
					.getChildAt(0);
			if (((QueryConceptTreeNodeModel) node1.getUserObject())
			...
			for (int i = 0; i < node.getChildCount(); i++) {
				DefaultMutableTreeNode anode = (DefaultMutableTreeNode) node
						.getChildAt(0);
				QueryConceptTreeNodeModel adata = (QueryConceptTreeNodeModel) anode
						.getUserObject();
	...
	private void addNodesFromXML(org.w3c.dom.Document resultDoc,
		...
		QueryConceptTreeNodeModel data = (QueryConceptTreeNodeModel) pnode
				.getUserObject();
		String c_xml = "";
		try {
