...
import edu.harvard.i2b2.patientSet.dataModel.TopPanelModel;
			...
			public void run() {
				...
				if (response != null) {
					...
					try {
						...
						QueryResultData nameNode = new QueryResultData();
						...
						if (status.equalsIgnoreCase("DONE")) {
							...
							for (int i = 0; i < dataModel
								...
								ArrayList<QueryConceptTreeNodeModel> nodelist = dataModel
										.getTreePanel(i).getItems();
								for (int j = 0; j < nodelist.size(); j++) {
									QueryConceptTreeNodeModel nodedata = nodelist
											.get(j);
