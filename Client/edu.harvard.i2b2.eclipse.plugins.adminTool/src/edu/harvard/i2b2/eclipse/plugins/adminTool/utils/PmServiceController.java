...
import edu.harvard.i2b2.crcxmljaxb.datavo.pm.UserType;
...
public class PmServiceController {
		...
		try {
			...
			else
			{
				...
				if(System.getProperty("webServiceMethod").equals("SOAP")) {
					...
					response = sendREST(new EndpointReference(projectURL + "getServices"), getUserInfoRequestString); 
				}
				if(response == null) {
	...
	public  String setUserPassword(String userid, PasswordType password, String projectURL, String domain, String newPassword, String username) throws Exception{
		String response = null;
		try {
			//////////////
			...
			//log.info("PM request: /n"+getUserInfoRequestString);
			if(System.getProperty("webServiceMethod").equals("SOAP")) { 
				response = sendSOAP(new EndpointReference(projectURL), requestString, "http://rpdr.partners.org/GetUserConfiguration", "GetUserConfiguration"); //$NON-NLS-1$ //$NON-NLS-2$
			}
			else {
	...
	@SuppressWarnings("rawtypes")
	public  String setUserPassword(String userid, PasswordType password, String projectURL, String domain, String newPassword) throws Exception{
		String response = null;
		try {
			//////////////
			/*GetUserRequestMessage reqMsg1 = new GetUserRequestMessage(userid, password, domain);
			...
			requestString = reqMsg.doBuildXML(newPassword);
			MessageUtil.getInstance().setRequest(
					"URL: " + projectURL + "getServices" + "\n" + requestString);
			//log.info("PM request: /n"+getUserInfoRequestString);
			if(System.getProperty("webServiceMethod").equals("SOAP")) { 
