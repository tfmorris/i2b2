...
public class CSVFileReader extends CSVFile {
	 ...
	 * @throws FileNotFoundException  If the file to be read does not exist
	 */
	public CSVFileReader(String inputFileName, char sep, char qual, String eol) throws FileNotFoundException {
		super(sep, qual, eol);
		in = new BufferedReader(new FileReader(inputFileName));
	}
	 ...
	 */
	public Vector<String> readFields(int count) throws IOException {
		...
		if (count < lineCount && eolQualifier.length() > 0 && !eolQualifier.equals("\n"))
		{
			String str = line;
			while ((str.indexOf(eolQualifier) == -1) && (!str.equals(eolQualifier)))
			{
				str = in.readLine();
				if (str == null) break;	
				str = str.replaceAll("\\|", "~");
