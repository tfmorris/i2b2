...
public class MonthSlider extends Slider {
	...
	public void wakeup(String status, MyDate minDate) {
		...
		if (status.equals("enabled")) {
			...
			maxClipWidth = 0;
			validMin = 0;
			validMax = sliderWidth - 2 * thumbWidth;
	...
	public boolean mouseDrag(Event e, int x, int y) {
		...
		case MINTHUMB:
			...
			if (rangeWidth < minOffset + monthRange)
				minOffset = rangeWidth - monthRange;
		...
		case MAXTHUMB:
			...
			if (rangeWidth < maxOffset + monthRange)
				maxOffset = rangeWidth - monthRange;
