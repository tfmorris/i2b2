...
public class MyDate {
	...
	public static int mdays[] = { 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30,
			31 };

	public MyDate copy() {
		MyDate temp = new MyDate(getMonth(), getDay(), getYear(), getHour(),
				getMin());
		return temp;
	}

	public boolean equals(MyDate checkIfEqual) {
		if (checkIfEqual.getHour() == getHour()
				&& checkIfEqual.getMin() == getMin()
				&& checkIfEqual.getMonth() == getMonth()
				&& checkIfEqual.getDay() == getDay()
				&& checkIfEqual.getYear() == getYear())
			return true;
	...
	/*
	 * Disable creation of d/m/y public MyDate(int m, int d, int y) { this.month
	 * = m; this.day = d; this.year = y; this.hour = 0; this.min = 0; }
	 */
	...
	public MyDate(String str) throws Exception {
		if (str.equals("today")) {
			...
			boolean yearFirst = (Record.yearFirst == true); // prevents
			// reference misuse?
			// System.out.println("yearFirst in MyDate " + (new
			// Boolean(yearFirst)).toString());
			...
			if (yearFirst) {
				...
				/*
				 * mm removed tokens = new StringTokenizer(str, "-"); token =
				 * tokens.nextToken(); year = (int)(new
				 * Integer(token.trim()).intValue());
				 * 
				 * token = tokens.nextToken(); month = (int)(new
				 * Integer(token.trim()).intValue());
				 * //System.out.println("month " + month);
				 * 
				 * token = tokens.nextToken(); day = (int)(new
				 * Integer(token.trim()).intValue()); //
				 * System.out.println("day " + day);
				 * 
				 * tokens = new StringTokenizer(token.trim(), ":"); token =
				 * tokens.nextToken(); hour = (int)(new
				 * Integer(token.trim()).intValue());
				 * 
				 * token = tokens.nextToken(); min = (int)(new
				 * Integer(token.trim()).intValue());
				 */
			} else {
				// mm-dd-yyyy hh:ss
				...
				/*
				 * mm removed 7/26/06 tokens = new StringTokenizer(str, "-");
				 * token = tokens.nextToken(); month = (int)(new
				 * Integer(token.trim()).intValue()); token =
				 * tokens.nextToken(); day = (int)(new
				 * Integer(token.trim()).intValue()); token =
				 * tokens.nextToken(); year = (int)(new
				 * Integer(token.trim()).intValue());
				 * 
	...
	public boolean after(MyDate when) {
		if (this.year > when.year)
			return true;
		else if (this.year < when.year)
			return false;
		else {
			if (this.month > when.month)
				return true;
			else if (this.month < when.month)
				return false;
			else {
				if (this.day > when.day)
					return true;
				else if (this.day < when.day)
					return false;
				else {
					if (this.hour > when.hour)
						return true;
					else if (this.hour < when.hour)
						return false;
					else {
						if (this.min > when.min)
							return true;
						else
							return false;
					}
	...
	public MyDate DateAfterDays(long days) { // Date in current object + days
		// number of days...
		return (DateFromStart(DaysFromStart(new MyDate(getMonth(), getDay(),
				getYear(), getHour(), getMin()))
				+ days));
	}
	...
	public long DateDiff(MyDate dateMax) {
		return (DaysFromStart(dateMax) - DaysFromStart(this)); // difference in
		// days between
		// two dates.
	}

	public long DaysFromStart(MyDate date) { // uses the current object any? I
		// don't think so...
		// start is 1/1/1
		...
		day = date.getDay(); // month - 1 since don't want
		// to count current month below

		// first compute days based on year and day
		days = (year - 1) * 365 + (year - 1) / 4 - (year - 1) / 100
				+ (year - 1) / 400 + day - 1; // year -1 since starting at 1,
		// extra
		// day every 4 years (leap), every 100 years (?), every 400 years. add
		// day to add in for the present month
		// subtract 1 (why?)

		for (i = 0; i < month; i++)
			days += mdays[i]; // mdays is number of days in each month (actually
		// 29 for feb!)
		// System.out.println("month " + month); //print

		if ((year % 4 == 0 && year % 100 != 0 || year % 400 == 0)
				&& (month >= 2))
			days++; // also adding for leaps here? not done
		// above?
	...
	/*
	 * public int HourDiff(MyDate dateMax){ return((dateMax.getHour() -
	 * this.hour +24)%24); }
	 */
	public long MinDiff(MyDate dateMax) {
		long i = DateDiff(dateMax) * 24 * 60 + dateMax.getTotalMin()
				- getTotalMin();
		if (i < 0)
			i = 0;
		return (i);
	...
	public void addDays(int daysToAdd) {
		...
		// important note: if add more than one month of days this is presently
		// wrong!

		if (daysToAdd > 31) {
			System.out
					.println("Warning: MyDate class doesn't yet support adding more than one month of days!");
		}

		//

		if (getDay() + daysToAdd > mdays[getMonth() - 1]) {// adding past end of
			// month? note:
			...
			setDay(getDay() + daysToAdd - (mdays[getMonth() - 1])); // again
			// watch
			// february. setting to number of days added after beginning of new
			// month
	...
	public void subtractDays(int daysToSubtract) {
		...
		// important note: if subtract more than one month of days this is
		// presently wrong!

		if (daysToSubtract > 31) {
			System.out
					.println("Warning: MyDate class doesn't yet support subtracting more than one month of days!");
		}

		//

		if (getDay() - daysToSubtract < 1) {// subtracting past end of month?
			// note:
			// assuming feb has 29 days! must fix
			...
			setDay(getDay() - daysToSubtract
					+ (getMonth() > 1 ? mdays[getMonth() - 2] : mdays[11])); // again
			// watch
			// february. setting to number of days subtracted after beginning of
			// new month
