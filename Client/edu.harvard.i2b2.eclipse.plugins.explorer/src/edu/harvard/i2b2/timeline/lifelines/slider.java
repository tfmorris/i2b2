...
public class Slider extends Panel { // implements newApplet?
	...
	protected int sliderWidth, sliderHeight, barWidth; // barWidth always
	// remains the original
	// width of the slider
	// (+thumbs?)
	public int thumbWidth, minThumbPos, maxThumbPos, rangeWidth, rangePos,
			thickness; // rangeWidth is present width of
	// rangeBar
	...
	protected Rectangle minThumbRect, maxThumbRect, rangeRect, minClipRect,
			maxClipRect;
	protected int currentPos = -1, minThumbState = UP, maxThumbState = UP,
			rangeState = UP, minClipState = UP, maxClipState = UP;
	protected int oldMinValue, oldMaxValue, oldRangeValue, minOffset,
			maxOffset, rangeOffset;
	protected int minClipPos = 0, minClipWidth = 0, maxClipPos,
			maxClipWidth = 0;
	protected int lowValue, highValue;
	protected Color llgray = new Color(220, 255, 220); // was (220,220,220) then
	// (220,220,0) color
	// under the slider
	...
	public Slider(int width, int height, MyDate today) {
		...
		rangeRect = new Rectangle(minThumbPos + thumbWidth, 0, rangeWidth,
				thickness);
		minClipRect = new Rectangle(0, 0, 0, 0);
		maxClipRect = new Rectangle(0, 0, 0, 0);
		maxClipPos = sliderWidth;
		lowValue = 0;
		highValue = sliderWidth - 2 * thumbWidth;
		...
		/*
		 * if(status.equals("enabled")) System.out.println("enabled");
		 */

		this.today = new MyDate(today.getMonth(), today.getDay(), today
				.getYear(), today.getHour(), today.getMin());
	...
	private void drawThumbImage(boolean raised) {
		if (thumbImage1 == null && raised) {
			...
			makeThumbArrays(); // should be called
			// "arrowArrays, vertices of arrows in thumb")
	...
	private void drawMinThumb(Graphics g, boolean raised) {
		if (raised)
			g.drawImage(thumbImage1, minThumbPos, 0, null);
		else
			g.drawImage(thumbImage2, minThumbPos, 0, null);
	}

	private void drawMaxThumb(Graphics g, boolean raised) {
		if (raised)
			g.drawImage(thumbImage1, maxThumbPos, 0, null);
		else
			g.drawImage(thumbImage2, maxThumbPos, 0, null);
	}
	...
	private void drawRange(Graphics g, boolean raised) {
		...
		if (raised)
			g.fillRect(rangePos + 2, 2, rangeWidth - 2, thickness - 4);
		else
			g.fillRect(rangePos + 1, 1, rangeWidth - 2, thickness - 2);
	}
	...
	private void drawBorderRect(Graphics g, int xRect, int yRect, int wRect,
			int hRect, boolean raised) {
		if (raised) {
	...
	private void makeThumbArrays() {
		xArray[0] = 3;
		yArray[0] = (thickness / 2);
		xArray[1] = (thumbWidth / 3 + 2);
		yArray[1] = 4;
		xArray[2] = (thumbWidth / 3 + 2);
		yArray[2] = (thickness / 3 + 2);
		xArray[3] = (thumbWidth * 2 / 3 - 1);
		yArray[3] = (thickness / 3 + 2);
		xArray[4] = (thumbWidth * 2 / 3 - 1);
		yArray[4] = 4;
		xArray[5] = (thumbWidth - 3);
		yArray[5] = (thickness / 2);
		xArray[6] = (thumbWidth * 2 / 3 - 1);
		yArray[6] = (thickness - 4);
		xArray[7] = (thumbWidth * 2 / 3 - 1);
		yArray[7] = (thickness * 2 / 3 - 1);
		xArray[8] = (thumbWidth / 3 + 2);
		yArray[8] = (thickness * 2 / 3 - 1);
		xArray[9] = (thumbWidth / 3 + 2);
		yArray[9] = (thickness - 4);
		xArray[10] = 3;
		yArray[10] = (thickness / 2);
	}

	private boolean inMinThumb(int x, int y) {
		if (minThumbRect.contains(x, y))
			return true;
		else
			return false;
	}
	...
	public boolean mouseDown(Event e, int x, int y) {
		...
		/*
		 * MyDate temp = new
		 * MyDate(CoordToDate(x).getMonth(),CoordToDate(x).getDay
		 * (),CoordToDate(x).getYear());
		 * 
		 * System.out.println("the date:"); temp.print();
		 */
		...
		if (inMinThumb(x, y)) {
			...
			if (minThumbState == UP) {
				...
				/*
				 * theApplet.showStatus(Integer.toString(validDateMin.getMonth())
				 * + "/" +Integer.toString(validDateMin. getDay() ) + "/" +
				 * Integer.toString(validDateMin.getYear()) + " | " +
				 * Integer.toString(validDateMax.getMonth()) + "/" +
				 * Integer.toString(validDateMax.getDay()) + "/" +
				 * Integer.toString(validDateMax.getYear()) );
				 * 
				 * 
				 * 
	...
	public boolean mouseDrag(Event e, int x, int y) {
		...
		case MINTHUMB:
			...
			if (rangeWidth < minOffset)
				minOffset = rangeWidth;
			else if (-minOffset > minClipWidth)
				minOffset = -minClipWidth;
			if (minOffset != 0) {
		...
		case MAXTHUMB:
			...
			if (rangeWidth < maxOffset)
				maxOffset = rangeWidth;
			else if (-maxOffset > maxClipWidth)
				maxOffset = -maxClipWidth;
			if (maxOffset != 0) {
	...
	public void adjust(int type, MyDate GivenDate1, MyDate GivenDate2,
			int rWinOffset) {
		validMin = 0;
		validMax = 0;

		MyDate Date1 = new MyDate(GivenDate1.getMonth(), GivenDate1.getDay(),
				GivenDate1.getYear(), GivenDate1.getHour(), GivenDate1.getMin());
		MyDate Date2 = new MyDate(GivenDate2.getMonth(), GivenDate2.getDay(),
				GivenDate2.getYear(), GivenDate2.getHour(), GivenDate2.getMin());
		...
		// theTicks[i] =
		// (int)Math.round((double)DateMin.MinDiff(currDate)*width/
		// DateMin.MinDiff(DateMax));

		// aScale.setScale(Date1,Date2);

		// double barWidth = (double) ( (double)barWidth/ (double)
		// (aScale.getDateMin()).MinDiff( aScale.getDateMax() ));
		// System.out.println("barWidth: " + barWidth);

		int start = (int) Math.round((double) dateMin.MinDiff(Date1) * barWidth
				/ dateMin.MinDiff(dateMax)); // scale.java where calculating the
		// ticks...
		...
		end += 2 * thumbWidth; // notice that in mouseup method for a click in
		// rangewidth (to reset) the end
		// goes to sliderWidth which is width of full range + 2*thumbWidth

		int oldMinValue = (int) Math.round((double) dateMin
				.MinDiff(validDateMin)
				* barWidth / dateMin.MinDiff(dateMax)); // scale.java where
		// calculating the
		// ticks...

		int oldMaxValue = (int) Math.round((double) dateMin
				.MinDiff(validDateMax)
				* barWidth / dateMin.MinDiff(dateMax)); // scale.java where
		// calculating the
		// ticks...
		...
		// start = start - thumbWidth; // still needed now that using rangeWidth
		// above?
		 ...
		 * minOffset = x - oldMinValue;
		 * 
		 * rangeWidth -= minOffset; minThumbPos += minOffset; rangePos +=
		 * minOffset; minClipWidth += minOffset; oldMinValue = x;
		 * minThumbRect.setBounds(minThumbPos, 0, thumbWidth, thickness);
		 * minClipRect.setBounds(minClipPos, 0, minClipWidth, thickness);
		 * rangeRect.setBounds(rangePos, 0, rangeWidth, thickness); repaint();
		 * lowValue = minThumbPos;
		 */
		...
		validDateMin = new MyDate(Date1.getMonth(), Date1.getDay(), Date1
				.getYear(), Date1.getHour(), Date1.getMin()); // set validMin to
		// start
		// instead?
		validDateMax = new MyDate(Date2.getMonth(), Date2.getDay(), Date2
				.getYear(), Date2.getHour(), Date2.getMin());

		/*
		 * currentPos = RANGEBAR; if(rangeState == UP){ rangeState = DOWN;
		 * oldRangeValue = 200; }
		 */
	...
	public MyDate CoordToDate(int start) {
		return (dateMin.DateAfterMins(Math.round((double) diff * start
				/ barWidth)));
	}
	...
	public void update(Graphics g) {
		...
		if (minThumbState == UP)
			...
			drawMinThumb(g, false);
		if (maxThumbState == UP)
			drawMaxThumb(g, true);
		else
			drawMaxThumb(g, false);
		drawMinClip(g);
		drawMaxClip(g);
		...
		if (status.equals("enabled")) {
			...
			for (int i = 0; i < aScale.n_ticks; i++) {
				if ((validMin < aScale.theTicks[i + 1])
				...
				g.drawString(aScale.theLabelString[i],
						(aScale.theTicks[i] + aScale.theTicks[i + 1]) / 2
								+ thumbWidth - strWidth / 2, fontMetrics
								.getHeight());
			}
		}
	}
