...
public class LoadRecord {
	...
	private void initLoadRecord(BufferedReader inputReader, String dataString) {
		...
		try {
			...
			// read one line
			while ((line = d.readLine()) != null) { // wait! this just reads a
				// line at time....

				// is it a key/value pair???
				// inputs are either key/value pairs or a single term

				if (!line.equals("")) { // don't want to read blank lines, fix
					// indendation sometime

					if (useCommas)
						line_tokens = new StringTokenizer(line, " ,", true); // also
					// loads
					// in
					// tokens,
					// note
					...
					if (token.equals("%facet")) {
						if (currentFacet != null) // 1/7/98
							currentFacet.layout(); // doing layout for previous
						// facet...
						...
						token = line_tokens.nextToken(); // reading facet color.
						// Not presently
						// used.
						...
						today = new MyDate(line_tokens.nextToken(","));
						if (max_date.before(today))
							max_date = new MyDate(today.getMonth(), today
									.getDay(), today.getYear(),
									today.getHour(), today.getMin());
					} else if (token.equals("%person")) {
						if (useCommas)
							token = line_tokens.nextToken(" ,"); // adding extra
						// for comma

						name = new String(readBlanks(line_tokens, ",")); // no
						// blank
						// to
						// read
						// names
						...
						gender = new String(readBlanks(line_tokens, " ,")); // added
						// blank
						// back
						// in
						// for
						...
						Record.yearFirst = false; // for different date form in
					// input file
				}
	...
	public Aggregate load_aggregate(boolean toplevel) { // toplevel
		...
		Aggregate agg = new Aggregate("aggregate"); // parameter is genrecord
		// type
		...
		try {
			...
			// remove spaces (for this
			// line) as a delimeter
			token = line_tokens.nextToken(); // reading in type. not presently
			// used
			...
			token = line_tokens.nextToken(); // reading in whether has a
			...
			for (int k = 0; k < ((summary) ? number + 1 : number); k++) {
				...
				while (token.equals("%c")) { // allow %c to occur inside %agg
					// section
					line = d.readLine();
					token = readBlanks(line_tokens);
				}
				...
				if (token.equals("%-")) {
					...
					if (couldBeComma.equals(","))
						end_date = new MyDate(start_date.getMonth(), start_date
								.getDay(), start_date.getYear(), start_date
								.getHour(), start_date.getMin());
					else
					...
					if (useCommas) {
						token = readBlanks(line_tokens, "p, "); // delimter is '
						// or , or space
						penWidth = Integer.parseInt(line_tokens.nextToken()); // reading
						// p
						// before
						// width*/
					} else
					...
					if (!token.equals("\"")) {
						...
						token = line_tokens.nextToken();
						while (!token.equals("\"")) {
							token = readBlanks(line_tokens, "\"="); // allow the
							// space in
							// the key
					...
					StoryRecord aRecord = new StoryRecord("story", cause,
							start_date, end_date, temp, penWidth, theUrl,
							attrList, line);
					...
					if (summary && k == 0) { // summary record
						...
						agg.addGen(aRecord);
						if (!summary)
							agg.setLabelString(cause); // 3/10/98 Julia, don't
						// seem need it. Orig
						// comment: o.k. since
						// right now using last
						// label
					}

					if (min_date.after(start_date))
						min_date = start_date;
					if (max_date.before(end_date))
						max_date = end_date;
				} // end if %-
				else if (token.equals("%agg")) {
	...
	public String readBlanks(StringTokenizer line_tokens, String delimeter) {
		...
		while (token.equals(" ")) {
			token = line_tokens.nextToken(delimeter); // read blank spaces at
			// beginning of line
			// plus first token?
		}
		return token;
