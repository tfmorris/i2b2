...
//$Id: XmlParser.java,v 1.7 2010/07/06 14:04:23 mem61 Exp $
...
public class XmlParser {
	 ...
	 */
	public void parse (String theString, String encoding)
	throws java.lang.Exception
	{
		byte stringsBytes[]=new byte[theString.length()];
		theString.getBytes(0,theString.length(),stringsBytes,0);
		ByteArrayInputStream theManufacturedInputStream = new ByteArrayInputStream(stringsBytes);
		doParse(null, null, theManufacturedInputStream, encoding);
	}
	...
	throws java.lang.Exception
	{
		doParse(systemId, publicId, null, encoding);
	}
	...
	throws java.lang.Exception
	{
		doParse(systemId, publicId, stream, encoding);
	}
	...
	throws java.lang.Exception
	{
		...
		pushURL("[document]", basePublicId, baseURI, baseInputStream,
				encoding);

		parseDocument();
//		if (readBufferPos < readBufferLength ||
//		(is != null && is.available() > 0)) {
//		error("end of XML document before end of input", null, null);
//		}
	...
	throws java.lang.Exception
	{
		...
		if (ids[1] != null) {
			pushURL("[external subset]", ids[0], ids[1], null, null);
	...
	throws java.lang.Exception
	{
		...
		if (type == ATTRIBUTE_ENUMERATED || type == ATTRIBUTE_NOTATION) {
			sEnum = dataBufferToString();
		}

		// Read the default value.
		requireWhitespace();
		parseDefault(elementName, name, type, sEnum);
	}
	 ...
	 */
	void parseDefault (String elementName, String name, int type, String sEnum)
	throws java.lang.Exception
	{
		...
		setAttribute(elementName, name, type, sEnum, value, valueType);
	}
	...
	throws java.lang.Exception
	{
		...
		for (int i = 0; i < ch.length; i++) {
			...
			// If we failed, push the saved characters
			// as if they were an internal entity.
			if (saved[i] != ch[i]) {
				if (i == 0) {
					unread(saved[0]);
				} else {
