...
public class ModifierNumericValueConstraintFrame extends javax.swing.JFrame {
	...
	public ModifierNumericValueConstraintFrame(ModifierData data_, KTable table_) {
		...
		if (!data.modifierValuePropertyData().okToUseValueFlag()) {
			jFlagRadioButton.setEnabled(false);
		}

		if (!data.modifierValuePropertyData().okToUseValue()) {
			jNumericRadioButton.setEnabled(false);
		}

		jUnitsComboBox.removeAllItems();
		for (int i = 0; i < data.modifierValuePropertyData().units.size(); i++) {
			jUnitsComboBox.addItem(data.modifierValuePropertyData().units.get(i));
		}
	...
	private void setPreviousValues() {
		if (data.modifierValuePropertyData().useNumericValue()) {
			jOperatorComboBox.setSelectedItem(data.modifierValuePropertyData().operator());

			jNumericRadioButton.setSelected(true);
			jNumericRadioButtonActionPerformed(null);
			jOperatorComboBoxActionPerformed(null);
			if (data.modifierValuePropertyData().operator().equalsIgnoreCase("BETWEEN")) {
				jLowValueTextField.setText(data.modifierValuePropertyData().lowValue());
				jHighTextField.setText(data.modifierValuePropertyData().highValue());
				jValuePanel.setVisible(false);
				jBetweenValuePanel.setVisible(true);
			} else {
				jValueTextField.setText(data.modifierValuePropertyData().value());
			}
		} else if (data.modifierValuePropertyData().useValueFlag()) {
			if (data.modifierValuePropertyData().value().equals("H")) {
				jFlagComboBox.setSelectedIndex(0);
				jFlagRadioButton.setSelected(true);
				jFlagRadioButtonActionPerformed(null);
			} else if (data.modifierValuePropertyData().value().equals("L")) {
				jFlagComboBox.setSelectedIndex(1);
				jFlagRadioButton.setSelected(true);
				jFlagRadioButtonActionPerformed(null);
			}
		}
		if (data.hasValue() && data.modifierValuePropertyData().unit() != null) {
			jUnitsComboBox.setSelectedItem(data.modifierValuePropertyData().unit());
		}
	...
	private void jOKButtonActionPerformed(java.awt.event.ActionEvent evt) {
		data.modifierValuePropertyData().raw = false;
		if (jNoValueRadioButton.isSelected()) {
			...
			data.modifierValuePropertyData().value(jValueTextField.getText());
			...
			// deal with between...
			if (operator.equalsIgnoreCase("between")) {
				data.modifierValuePropertyData().lowValue(jLowValueTextField.getText());
				data.modifierValuePropertyData().highValue(jHighTextField.getText());
				data.valueName("between " + data.modifierValuePropertyData().lowValue()
						+ " and " + data.modifierValuePropertyData().highValue());
			} else {
		...
		if (jUnitsComboBox.getSelectedItem() != null) {
			data.modifierValuePropertyData().unit((String) jUnitsComboBox.getSelectedItem());
		}
