...
import edu.harvard.i2b2.explorer.ui.WebColorDialog;
...
class KTableCellEditorColor extends KTableCellEditor {
	...
	public void open(KTable table, int row, int col, Rectangle rect) {
		...
		Object color = m_Table.getModel().getContentAt(6, row);
		if (color != null)
	...
	public void setColor(RGB rgb) {
		 ...
		 * if (i2Model.isColorUsable(rgb)) { // Dispose the old color, create
		 * the // new one, and set into the label m_Color = rgb; try { Display
		 * dplay = m_Table.getDisplay(); Color colBack = new Color(dplay, rgb);
		 * m_Label.setBackground(colBack); // colBack.dispose(); } catch
		 * (Exception e) { System.out.println(e.getMessage()); } } else {
		 * MessageBox mBox = new MessageBox(m_Table.getShell(),
		 * SWT.ICON_INFORMATION | SWT.OK); mBox.setText("Invalid Color");
		 * mBox.setMessage
		 * ("The color you picked is not supported.  Please choose another color."
		 * ); mBox.open(); }
		 */
		close(true);
	}
