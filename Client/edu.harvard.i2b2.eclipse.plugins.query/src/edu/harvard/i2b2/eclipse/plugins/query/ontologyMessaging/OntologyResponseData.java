...
abstract public class OntologyResponseData {
	...
	public static final String THIS_CLASS_NAME = OntologyResponseData.class
			.getName();
	...
	public StatusType processResult(String response) {
		...
		try {
			...
			ResponseHeaderType responseHeader = respMessageType
					.getResponseHeader();
	...
	public ConceptsType doReadConcepts() {
		...
		try {
			...
			if (bodyType != null)
				concepts = (ConceptsType) helper.getObjectByClass(bodyType
						.getAny(), ConceptsType.class);
		} catch (JAXBUtilException e) {
			log.error(e.getMessage());
			;
		}
	...
	public String getXMLString(ConceptType concept) throws Exception {
		...
		try {
			QueryJAXBUtil.getJAXBUtil().marshaller(concept, strWriter);
		} catch (JAXBUtilException e) {
