 ...
 * @author  wp066
 */
public class QueryConstrainFrame extends javax.swing.JFrame {
	
	private QueryConceptTreePanel parentPanel;
	
    /** Creates new form QueryConstrainFrame */
    public QueryConstrainFrame(QueryConceptTreePanel parent) {
        initComponents();
        //mode = Mode;
        parentPanel = parent;
        
        if(parentPanel.data().startTime() != -1) {
        	this.jFromDateCheckBox.setSelected(true);
        	this.jFromDateField.setEnabled(true);
        	this.jFromDateField.setEditable(true);
        	this.jFromDateField.setDateInMillis(parentPanel.data().startTime());
        }
        else {
        	TimeZone pdt = TimeZone.getTimeZone("EST"); 
        	Calendar calendar = new GregorianCalendar(pdt);
        	calendar.set(1979, 11, 1);
        	this.jFromDateField.setDateInMillis(calendar.getTimeInMillis());
        }
        
        if(parentPanel.data().endTime() != -1) {
        	this.jToDateCheckBox.setSelected(true);
        	this.jToDateField.setEnabled(true);
        	this.jToDateField.setEditable(true);
        	this.jToDateField.setDateInMillis(parentPanel.data().endTime());
        }
        
        if(parent.getClass().getName().equals(QueryConceptTreePanel.class.getName())) {
        	jExcludeSubitemCheckBox.setVisible(false);
        	jExcludeSubitemCheckBox.setEnabled(false);
        	jOKButton.setBounds(110, 130, 49, 23);
        	jCancelButton.setBounds(240, 130, 67, 23);
        	setSize(430, 200);
        }
        else {
        	setSize(430, 250);
    ...
    private void initComponents() {
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder("Constrain by dates"));
        ...
        jFromDateCheckBox.addActionListener(new java.awt.event.ActionListener() {
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder("Contrain by priority in visit"));
        jPrincipalCheckBox.setText("Items classified as principal diagnoses or procedures.");
        jPrincipalCheckBox.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jPrincipalCheckBox.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jPanel4.add(jPrincipalCheckBox);
        jPrincipalCheckBox.setBounds(30, 20, 320, 20);
        jPrincipalCheckBox.setSelected(true);

        jSecondaryCheckBox.setText("Items classified as secondary diagnoses or procedures.");
        jSecondaryCheckBox.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jSecondaryCheckBox.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jPanel4.add(jSecondaryCheckBox);
        jSecondaryCheckBox.setBounds(30, 50, 320, 20);
        jSecondaryCheckBox.setSelected(true);

        jAdmissionCheckBox.setText("Items classified as admission diagnoses.");
        jAdmissionCheckBox.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jAdmissionCheckBox.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jPanel4.add(jAdmissionCheckBox);
        jAdmissionCheckBox.setBounds(30, 80, 320, 20);
        jAdmissionCheckBox.setSelected(true);

        getContentPane().add(jPanel4);
        jPanel4.setBounds(10, 110, 400, 120);

        jExcludeSubitemCheckBox.setText("Do not include the sub-items in this folder.");
        jExcludeSubitemCheckBox.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        ...
        jOKButton.setBounds(110, 150, 49, 23);
        ...
        jCancelButton.setBounds(240, 150, 67, 23);
    ...
    private void jOKButtonActionPerformed(java.awt.event.ActionEvent evt) {
    	...
    	if(this.jFromDateCheckBox.isSelected()) {
    		...
    		parentPanel.data().startMonth(fromCalendar.get(Calendar.MONTH)+1);
    		parentPanel.data().startDay(fromCalendar.get(Calendar.DAY_OF_MONTH));
    	...
    	if(this.jToDateCheckBox.isSelected()) {
    		...
    		parentPanel.data().endMonth(toCalendar.get(Calendar.MONTH)+1);
    		parentPanel.data().endDay(toCalendar.get(Calendar.DAY_OF_MONTH));
    	...
    	parentPanel.data().includePrincipleVisit(this.jPrincipalCheckBox.isSelected());
    	parentPanel.data().includeSecondaryVisit(this.jSecondaryCheckBox.isSelected());
    	parentPanel.data().includeAdmissionVisit(this.jAdmissionCheckBox.isSelected());
    	
    	setVisible(false);
    	
    	java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
            	if(parentPanel.data().startTime() == -1 && parentPanel.data().endTime() == -1) {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new QueryConstrainFrame(null).setVisible(true);
            }
        });
    }
