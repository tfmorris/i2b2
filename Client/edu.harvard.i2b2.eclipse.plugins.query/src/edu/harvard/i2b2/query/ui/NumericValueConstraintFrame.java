...
public class NumericValueConstraintFrame extends javax.swing.JFrame {
		...
		if (jNoValueRadioButton.isSelected()) {
			...
			if(ud != null && ud.needConvertion()) {
				if(!val.equals("")) {
					...
					data.valuePropertyData().value(""+nval*ud.mFactor());
				}
			}
			else {
				data.valuePropertyData().value(val);
			}
			...
			// deal with between...
			if (operator.equalsIgnoreCase("between")) {
				data.valuePropertyData().lowValue(""+Double.parseDouble(jLowValueTextField.getText())*ud.mFactor());
				data.valuePropertyData().highValue(""+Double.parseDouble(jHighTextField.getText())*ud.mFactor());
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				new NumericValueConstraintFrame(null).setVisible(true);
			}
		});
	}
