/*
 * $Id: JDatePicker.java,v 1.4 2010/01/12 22:05:06 wp066 Exp $
 *
 ...
 * Class: JDatePicker
 * 
 * Based on JXDatePicker from JDNC. Several modifications have been made for it
 * being used in QueryConstrainFrame class. (wp066)
 * 
 */
 ...
 * appears when the button is pressed. The selection from the calendar component
 * will be displayed in editable field. Values may also be modified manually by
 * entering a date into the editable field using one of the supported date
 * formats.
 * 
...
public class JDatePicker extends JComponent {
	...
	private JCheckBox associatedCheckBox = null;

	/**
	 * Create a new date picker using the current date as the initial selection.
	 */
	public JDatePicker(JCheckBox checkBox) {
	 ...
	 */
	public JDatePicker(long millis) {
		_dateField = new JFormattedTextField(new JXDatePickerFormatterFactory());
		_dateField.setName("dateField");
		_dateField.setBorder(null);
		_dateField.setFont(new Font("Tahoma", Font.PLAIN, 11));
	...
	public void setEditable(boolean editable) {
		if (editable) {
			this._dateField.setEditable(true);
		} else {
	...
	protected void fireActionPerformed() {
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == ActionListener.class) {
				if (e == null) {
					e = new ActionEvent(JDatePicker.this,
	...
	public void doLayout() {
		...
		_dateField.setBounds(insets.left, insets.bottom, width
				- _popupButtonWidth, height);
		_popupButton.setBounds(width - _popupButtonWidth + insets.left,
				insets.bottom, _popupButtonWidth, height);
	}
	...
	private class Handler implements MouseListener, MouseMotionListener {
		...
		public void toggleShowPopup() {
			...
			if (!_popup.isVisible()) {
				...
				DateSpan span = new DateSpan((java.util.Date) _dateField
						.getValue(), (java.util.Date) _dateField.getValue());
				_monthView.setSelectedDateSpan(span);
				_monthView.ensureDateVisible(((Date) _dateField.getValue())
						.getTime());
				// Point loc = _dateField.getLocationOnScreen();
				_popup.show(JDatePicker.this, 0, JDatePicker.this.getHeight());
			} else {
	...
	private class JXDatePickerPopup extends JPopupMenu implements
		...
		public void actionPerformed(ActionEvent ev) {
			...
			if ("MONTH_VIEW" == command) {
				...
				_popup.setVisible(false);
				associatedCheckBox.setSelected(true);
				fireActionPerformed();
			} else if ("PREVIOUS_MONTH" == command) {
				_monthView.setFirstDisplayedDate(DateUtils
						.getPreviousMonth(_monthView.getFirstDisplayedDate()));
			} else if ("NEXT_MONTH" == command) {
				_monthView.setFirstDisplayedDate(DateUtils
						.getNextMonth(_monthView.getFirstDisplayedDate()));
			} else if ("TODAY" == command) {
				DateSpan span = new DateSpan(System.currentTimeMillis(), System
						.currentTimeMillis());
