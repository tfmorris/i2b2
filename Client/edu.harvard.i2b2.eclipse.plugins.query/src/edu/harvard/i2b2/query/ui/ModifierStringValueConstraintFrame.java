...
public class ModifierStringValueConstraintFrame extends javax.swing.JFrame {
	...
	public ModifierStringValueConstraintFrame(GroupPanel parent) {
		parent_ = parent;
		...
		if ((!(UserInfoBean.getInstance().isRoleInProject("DATA_DEID")))
				&& ((ModifierData)parent_.currentData()).modifierValuePropertyData().isLongText()) {
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				new ModifierStringValueConstraintFrame(null).setVisible(true);
			}
		});
	}
