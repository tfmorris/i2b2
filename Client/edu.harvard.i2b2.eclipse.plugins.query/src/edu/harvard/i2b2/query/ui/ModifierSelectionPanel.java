...
public class ModifierSelectionPanel extends javax.swing.JPanel {
	...
	public ModifierSelectionPanel(MainPanelModel model) {
	...
	private void initComponents() {
		...
		jAnytimeRadioButton = new JRadioButton("Dose");
		jAnytimeRadioButton
		...
		((JRadioButton) dm.getValueAt(0, 0)).setEnabled(false);
		//((JRadioButton) dm.getValueAt(0, 0)).setSelected(true);
		((JRadioButton) dm.getValueAt(1, 0)).setEnabled(false);
		((JRadioButton) dm.getValueAt(2, 0)).setEnabled(false);
		...
		jAnalysisTable.getColumn("Name").setCellRenderer(
				new RadioButtonRendererM());
		jAnalysisTable.getColumn("Name").setCellEditor(
				new RadioButtonEditorM(new JCheckBox()));
...
class RadioButtonRendererM implements TableCellRenderer {
...
class RadioButtonEditorM extends DefaultCellEditor implements ItemListener {
	private JRadioButton button;
	private JTable table;

	public RadioButtonEditorM(JCheckBox checkBox) {
		super(checkBox);
	}
