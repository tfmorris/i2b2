 ...
 *     
 */
package edu.harvard.i2b2.patientMapping.serviceClient;
...
import edu.harvard.i2b2.patientMapping.dataModel.PDOItem;
import edu.harvard.i2b2.patientMapping.dataModel.PDORequestMessageModel;
import edu.harvard.i2b2.patientMapping.dataModel.PDOResponseMessageModel;
import edu.harvard.i2b2.patientMapping.dataModel.PDOValueModel;
...
public class PatientMappingQueryClient {
	private static final Log log = LogFactory.getLog(PatientMappingQueryClient.class);
	...
	private static String getPDOServiceName() {
		if(System.getProperty("UseIMCell") != null 
			...
			return UserInfoBean.getInstance().getCellDataUrl("IM") + "pdorequest";
		}
		else {
	...
	public static String getlldString(ArrayList<TimelineRow> tlrows,
		...
		try {
			...
			return new PatientMappingFactory().generateTimelineData(result, tlrows,
	...
	public static String getPidString(String id, int min, int max) {
		...
		try {
			...
			//	pid = patientRefId;
			//}
			String xmlStr = pdoFactory.requestXmlMessage(id, min, max);
			// explorer.lastRequestMessage(xmlStr);
	...
	public static String getPidStringFromPid(String id) {
		...
		try {
			...
			//	pid = patientRefId;
			//}
			String xmlStr = pdoFactory.requestXmlMessagePid(id);
			// explorer.lastRequestMessage(xmlStr);
	...
	private static void testWriteTableFile(String result) {
		...
		try {
			...
			PatientMappingFactory.append(f, "PatientNumber,Race,Sex,Age,Dead\n");
			PatientMappingFactory.append(f, resultFile.toString());
			f.close();
